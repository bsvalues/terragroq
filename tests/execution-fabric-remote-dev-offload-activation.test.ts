...
describe("TerraFusion remote development one-run activation", () => {
  ...
  it("accepts only a fresh root policy proof for the exact locked activation identity", () => {
    const proof:any={schemaVersion:1,status:"ROOT_NO_SUDO_VERIFIED",activationId:"remote-dev-offload-v1-issue-734-single-use-003",authorityReference:"issue-734-terrafusion-remote-dev-single-use-003",runId:"bd39dfc5-ef3b-4789-a5e6-095385330424",machineIdSha256:"a".repeat(64),bootId:"11111111-1111-4111-8111-111111111111",activationHostSha256:"b".repeat(64),authoritySha256:"c".repeat(64),account:"williamos-fabric",passwordStatus:"L",sudoStatus:0,sudoStdout:"User williamos-fabric is not allowed to run sudo on aegis.\n",sudoStderr:"",issuedAt:"2026-08-13T06:06:00.000Z",expiresAt:"2026-08-13T06:11:00.000Z"}
    ...
    expect(inspectRootNoSudoProof(proof,"2026-08-13T06:06:30.000Z",expected)).toMatchObject({status:"ROOT_NO_SUDO_VERIFIED"})
    for(const value of [{...proof,passwordStatus:"P"},{...proof,sudoStatus:1},{...proof,sudoStdout:"sudo: a password is required\n"},{...proof,expiresAt:"2026-08-13T06:06:30.000Z"},{...proof,extra:true}])expect(inspectRootNoSudoProof(value,"2026-08-13T06:06:30.000Z",expected)).toMatchObject({status:"BLOCKED"})
  })
})
