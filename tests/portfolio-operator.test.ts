...
describe("portfolio operator", () => {
  ...
  it("selects WO-MAO-056 and WO-MAO-057 after concurrent certification lanes", () => {
    const portfolio = getPortfolioOperatorProgram()
    const multiAgentOperator = portfolio.backlog[0]

    expect(buildLoopPacket(multiAgentOperator)).toMatchObject({
      activeWorkOrder: "WO-MAO-056",
      eligibleWorkOrders: ["WO-MAO-056", "WO-MAO-057"],
  ...
  it("generates bounded goal, loop, and Work Order packets", () => {
    ...
    expect(workOrders[55]).toMatchObject({ workOrderId: "WO-MAO-056", status: "READY", riskClass: "R2" })
    expect(workOrders[56]).toMatchObject({ workOrderId: "WO-MAO-057", status: "READY", riskClass: "R2" })
