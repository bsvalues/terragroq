...
describe("portfolio operator", () => {
  ...
  it("selects WO-MAO-038 after the branch delivery proof", () => {
    const portfolio = getPortfolioOperatorProgram()
    const multiAgentOperator = portfolio.backlog[0]

    expect(buildLoopPacket(multiAgentOperator)).toMatchObject({
      activeWorkOrder: "WO-MAO-038",
      eligibleWorkOrders: ["WO-MAO-038"],
  ...
  it("generates bounded goal, loop, and Work Order packets", () => {
    ...
    expect(workOrders[37]).toMatchObject({ workOrderId: "WO-MAO-038", status: "READY", riskClass: "R3" })
    expect([30, 31, 34, 35, 36, 37, 38].map((number) => workOrders[number - 1].status))
