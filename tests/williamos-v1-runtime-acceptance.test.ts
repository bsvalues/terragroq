...
describe("WilliamOS V1 Issue #448 acceptance campaign", () => {
  ...
  it("validates high-cardinality mutation histories without spreading an unbounded timestamp array", () => {
    ...
    state.executions["outcome-50000"].checkpoint.recordedAt = "2026-07-23T17:59:00.001Z"
    expect(validateHostState(state, { now }).code).toBe("HOST_STATE_CAUSALITY_INVALID")
  })
  ...
  it("accepts historical terminal state but fails closed on invalid time, owner relay, duplicate fencing, and unreleased work", () => {
    ...
    const deferredExpiry = hostState()
    deferredExpiry.executions["outcome-1"].checkpoint.state = "DEFERRED_PROVIDER_UNAVAILABLE"
    deferredExpiry.executions["outcome-1"].lease.status = "DEFERRED"
    deferredExpiry.executions["outcome-1"].lease.expiresAt = "2026-07-23T18:05:00.000Z"
    Object.assign(deferredExpiry.executions["outcome-1"].lease, {
      deferredAt: fresh,
      deferReason: "PROVIDER_UNAVAILABLE",
    })
    expect(validateHostState(deferredExpiry, { now }).code).toBe("PASS")
