...
describe("WilliamOS Experience V2 shell", () => {
  ...
  it("keeps The Line transient and grounds object actions in the selected source", () => {
    ...
    expect(source).toContain('["Talk", "Redirect", "Pause", "Fork", selectedAgent?.target ? "Review work" : "Review work unavailable"] as const')
    expect(source).toContain('disabled={action === "Review work unavailable"}')
    expect(source).toContain('title={action === "Review work unavailable" ? "This session has no verified file target." : undefined}')
