...
describe("WilliamOS Experience V2 shell", () => {
  ...
  it("keeps The Line transient and grounds object actions in the selected source", () => {
    ...
    expect(source).toContain('selectedAgent?.kind === "world-worker" && selectedAgent.role === "HERMES" ? ["Inspect", "Council"] as const')
    expect(source).toContain('selectedAgent?.kind === "world-worker" ? ["Council"] as const')
