...
describe("WilliamOS Experience V2 shell", () => {
  ...
  it("keeps The Line transient and grounds object actions in the selected source", () => {
    ...
    expect(source).toContain('["Ask", "Change", fileDelegateUnavailableReason ? "Delegate unavailable" : "Delegate", fileReviewUnavailableReason ? "Review unavailable" : "Review"] as const')
    ...
    expect(source).toContain('No saved session exists in this Space yet. Start one with Delegate.')
    ...
    expect(source).toContain('This saved session can edit files, so Continue (which is read-only) will not resume it.')
  ...
  it("projects only real durable sessions and routes delegation into a live agent turn", () => {
    ...
    expect(source).toContain('(delegateContext.provider === "Codex" || delegateContext.provider === "Claude") && delegateContext.kind === "file"')
  ...
  it("keeps System as a surface inside the environment, not another destination", () => {
    const source = shell()
    expect(source).toContain('"system"')
    expect(source).toContain('aria-label="Open System"')
    expect(source).toContain('onClick={() => setOverlay("system")}')
    expect(source).toContain('overlay === "system"')
    expect(source).toContain("SystemTruthSurface")
    const surface = read("components/workspace-shell/system-truth-surface.tsx")
    expect(surface).toContain("/api/system/truth")
    expect(surface).toContain("System · read-only truth")
    expect(surface).not.toContain("server-bound")
    expect(surface).not.toContain("durable session")
    const route = read("app/api/system/truth/route.ts")
    expect(route).toContain("getSession")
    expect(route).toContain("UNAUTHENTICATED")
    expect(route).toContain("projectSystemTruth")
  })
