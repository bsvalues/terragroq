...
describe("WilliamOS Experience V2 shell", () => {
  ...
  it("keeps The Line transient and grounds object actions in the selected source", () => {
    ...
    expect(source).toContain('aria-label={lineMode === "change" ? "Change" : lineMode === "review" ? "Review" : lineMode === "fork" ? "Fork session" : "The Line"}')
    ...
    expect(source).toContain('["Review", "Improve", "Challenge", "Merge unavailable"] as const')
    ...
    expect(source).toContain('openLine(`${action} the exact current patch for the selected file.`)')
    ...
    expect(source).toContain('disabled={action === "Review work unavailable" || action === "Pause unavailable" || action === "Fork unavailable" || action === "Merge unavailable" || action === "Continue unavailable"}')
