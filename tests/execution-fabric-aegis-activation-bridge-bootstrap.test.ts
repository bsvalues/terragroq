...
describe("AEGIS activation bridge bootstrap authority", () => {
  ...
  it("upgrades only the exact reviewed activation-host predecessor", () => {
    const destination="/usr/local/libexec/williamos-aegis-remote-dev-activation-host.mjs", current="d467800c3f288e13174d6a12c54199fbff0047e5a8e1b7ced09372dfea78562c"
    expect(inspectBridgeDestinationState(destination,"ba97c1ff4cc416e7aa27e6121a090ae37430e01e5b0197e706142c0979a1aa9b","0555",current)).toBe("EXACT_PREDECESSOR")
    ...
    expect(inspectBridgeDestinationState("/usr/local/libexec/other","ba97c1ff4cc416e7aa27e6121a090ae37430e01e5b0197e706142c0979a1aa9b","0555",current)).toBe("DRIFT")
    expect(inspectBridgeDestinationState(destination,"ba97c1ff4cc416e7aa27e6121a090ae37430e01e5b0197e706142c0979a1aa9b","0777",current)).toBe("DRIFT")
  })

  it("accepts only the exact first bridge receipt as a bounded successor predecessor", () => {
    expect(inspectBridgeReceiptState("41b55d3f250cfb6524d203e275750d49206f91ecbbb493971ad278fec836f339","0".repeat(64))).toBe("EXACT_PREDECESSOR")
    expect(inspectBridgeReceiptState("5c70148840df17de905198c3aceb1f16d5a5efe7a8b25291c7b6d0bfee9b7293","0".repeat(64))).toBe("DRIFT")
