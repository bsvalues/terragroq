...
describe("four-primary cockpit navigation", () => {
  ...
  it("uses the same four destinations in the compact Home rail", () => {
    ...
    const rail = frame.slice(
      frame.indexOf("const HOME_RAIL_DESTINATIONS"),
      frame.indexOf("])\n\nexport function AppShellFrame"),
    )

    for (const href of ['"/"', '"/projects"', '"/activity"', '"/runtime"']) {
  ...
  it("keeps supporting routes out of primary navigation without deleting them", () => {
    expect(navItems.map((item) => item.href)).not.toEqual(
      expect.arrayContaining(["/work-orders", "/audit", "/brain-council", "/goal-console", "/chat"]),
    )
