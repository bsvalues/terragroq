...
function validEnvelope() {
  return {
    ...
    allowedActions: ["RUN_VALIDATION", "WRITE_RESERVED_PATHS", "READ_REPOSITORY"],
...
describe("hosted-team dispatch envelope", () => {
  ...
  it.each([
    ...
    ["merge action when merge disabled", (value: ReturnType<typeof validEnvelope>) => { value.mergeMode = "NO_MERGE"; value.allowedActions.push("MERGE_ELIGIBLE_PR") }, "DISPATCH_ENVELOPE_CONTRADICTION_WALL", "mergeMode"],
  ...
  it("provides a machine-readable CLI success and typed failure", () => {
    ...
    expect(success).toMatchObject({ ok: true, code: "DISPATCH_ENVELOPE_VALID" })
