...
describe("transactional durable outcome queue source", () => {
  ...
  it("uses the exact quoted schema contract and deterministic ordering", () => {
    ...
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`q."authorityLevel" = 'A2_WRITE_OWN'`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`exact_execution_grant."expiresAt" IS NOT NULL`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`cardinality(exact_execution_grant."allowedActions") = 1`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`exact_execution_grant."allowedActions"[1] = 'outcome:execute'`)
    ...
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`FROM "workbench_thread_source" AS global_execution_root`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`global_execution_root."sourceId" = q."outcomeKey"`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`execution_thread."userId" = q."userId"`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`execution_project.lifecycle = 'active'`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`min(primary_repo."canonicalIdentity") = 'bsvalues/terragroq'`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`primary_repo.relationship = 'primary-repo'`)
    expect(OUTCOME_QUEUE_SQL.acquire).toContain(`HAVING count(*) = 1`)
    expect(OUTCOME_QUEUE_SQL.reclaimAcquisition).toContain(`FROM "workbench_thread_source" AS execution_root`)
    expect(OUTCOME_QUEUE_SQL.revalidateAcquisition).toContain(`FROM "workbench_thread_source" AS execution_root`)
    expect(OUTCOME_QUEUE_SQL.renewLease).toContain(`FROM "workbench_thread_source" AS execution_root`)
    expect(OUTCOME_QUEUE_SQL.bindWorkOrder).toContain(`FROM "workbench_thread_source" AS execution_root`)
    expect(OUTCOME_QUEUE_SQL.verifyBoundWorkOrder).toContain(`FROM "workbench_thread_source" AS execution_root`)
    expect(OUTCOME_QUEUE_SQL.complete).toContain(`FROM "workbench_thread_source" AS execution_root`)
  ...
  it("reclaims an expired committed validation recovery before replaying local state", async () => {
    ...
    expect(OUTCOME_QUEUE_SQL.readValidationRecoveryCandidate)
      .toContain(`$9::text IS NOT NULL`)
    expect(OUTCOME_QUEUE_SQL.readValidationRecoveryCandidate)
      .toContain(`$10::text IS NOT NULL`)
    expect(OUTCOME_QUEUE_SQL.readValidationRecoveryCandidate)
      .toContain(`$11::timestamptz IS NOT NULL`)
    expect(OUTCOME_QUEUE_SQL.readValidationRecoveryCandidate)
      .toContain(`$12::timestamptz IS NOT NULL`)
