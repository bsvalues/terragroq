...
describe("inactive Hermes-mediated remote development controller", () => {
  it("owns activation start and terminal settlement in the reviewed relay", () => {
    ...
    expect(source).toContain("$started=InvokeActivation ([ordered]@{action='start'}) 'ACTIVATION_START_FAILED';if($started.Value.payload.runId")
    expect(source).toContain("$script:activationStarted=$true;$state.inFlightOperation=$relay.operation;SaveState")
    expect(source).not.toContain("$script:activationStarted=$true;$started=InvokeActivation")
    expect(source).toContain("-F NUL -l williamos-fabric -o IdentitiesOnly=yes")
    expect(source).toContain("$trustedOmenIdentity = 'OMEN\\bsval'")
    expect(source).toContain("$trustedOmenMachine = 'OMEN'")
    expect(source).toContain("$trustedOmenPrivateKey = 'C:\\Users\\bsval\\.ssh\\id_ed25519'")
    expect(source).toContain("$trustedOmenClientFingerprint = 'SHA256:yKY2L2DIR7KaYtgr4Vm5VXQrlzZmGk82GmU+2ARAWG8'")
    expect(source).toContain("[Security.Principal.WindowsIdentity]::GetCurrent().Name")
    expect(source).toContain("[Environment]::MachineName")
    expect(source).toContain("[IO.Directory]::GetAccessControl($keyParentPath)")
    expect(source).toContain("[IO.File]::GetAccessControl($trustedOmenPrivateKey)")
    expect(source).toContain("$expectedParentAccess = @(")
    expect(source).toContain("ssh-keygen.exe")
    expect(source.split("C:\\Users\\bs\\.ssh\\id_ed25519")).toHaveLength(3)
    expect(source).not.toMatch(/\$env:(?:HOME|USERPROFILE|USERNAME)/i)
    expect(source).toContain("$hermesSsh = @('-F','NUL','-l','bs','-p','22'")
    expect(source).toContain("$hermesScp = @('-q','-F','NUL','-P','22'")
    expect(source).toContain("'HostKeyAlias=hermes','192.168.88.9'")
    expect(source).toContain("'bs@192.168.88.9:'+$remoteTransportPath")
    expect(source).toContain('-i "C:\\Users\\bs\\.ssh\\id_ed25519"')
    expect(source).toContain("$hermesKeyAcl=[IO.File]::GetAccessControl($hermesKey)")
    expect(source).toContain("SHA256:mvFpy/9iEGiFx2zYp5wuzEcURmGyGNSRJYNi0ocEWiM")
    expect(source).not.toContain("'hermes','powershell.exe'")
    expect(source).toContain("-o HostKeyAlias=aegis 192.168.88.6'")
    expect(source).toContain("$ap.Arguments=$aegisSsh+' activation '")
    expect(source).toContain("$mintPsi.Arguments=$aegisSsh+' activation '")
    expect(source).toContain("$psi.Arguments=$aegisSsh+' /usr/bin/node /usr/local/libexec/williamos-aegis-remote-dev-network-launcher.mjs '")
    expect(source).not.toContain("$knownHosts+'\" aegis activation '")
    expect(source.indexOf("$started=InvokeActivation ([ordered]@{action='start'})")).toBeLessThan(source.indexOf("$state.inFlightOperation=$relay.operation;SaveState"))
...
describe.skip("future-activation Hermes-mediated remote development controller", () => {
  it("contacts only Hermes with BatchMode, a finite timeout, and an encoded independently validating relay", () => {
    ...
    expect(log).toContain("C:\\Users\\bsval\\.ssh\\id_ed25519")
