...
const identity = { pullRequest: 1117, state: "OPEN" as const, headSha: head, pullRequestBaseSha: "0".repeat(40), baseRefSha: base, baseSha: base, paths }
const evidence = {
...
describe("prospective exact-artifact adoption", () => {
  ...
  it("fails closed when head, paths, validation, review, or current Space authority drift", async () => {
    ...
    for (const override of [
      ...
      { inspectArtifactIdentity: vi.fn().mockResolvedValue({ ...identity, pullRequestBaseSha: "5".repeat(40) }) },
