...
describe("Execution Fabric single-shot dispatch store", () => {
  ...
  it("requires the pre-provisioned genesis and rejects deletion or substitution", () => {
    const { statePath } = fixture()
    const initializer = createSingleShotDispatchStore(statePath, {
      storeId: "resident-hermes-proof",
    })
    initializer.initialize()
    const genesisSha256 = initializer.read().genesis_sha256
    const bound = createSingleShotDispatchStore(statePath, {
      storeId: "resident-hermes-proof",
      requireExisting: true,
      expectedGenesisSha256: genesisSha256,
    })
    expect(bound.read().genesis_sha256).toBe(genesisSha256)

    expect(() => createSingleShotDispatchStore(statePath, {
      storeId: "resident-hermes-proof",
      requireExisting: true,
      expectedGenesisSha256: sha("wrong-genesis"),
    }).read()).toThrowError(expect.objectContaining({ code: "DISPATCH_STATE_GENESIS_MISMATCH" }))

    fs.rmSync(statePath)
    expect(() => bound.read()).toThrowError(expect.objectContaining({ code: "DISPATCH_STATE_GENESIS_MISSING" }))
  })
})
