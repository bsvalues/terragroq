...
describe("AEGIS Issue #595 SSH coexistence one-shot root repair", () => {
  ...
  it("requires the external pinned launcher before reading a valid authority path", () => {
    ...
    expect(launcherBytes.toString("utf8")).toContain("[ -f /proc/self/fd/9 ]")
    expect(launcherBytes.toString("utf8")).toContain("stat -Lc '%u:%g:%a' /proc/self/fd/9")
    expect(launcherBytes.toString("utf8")).not.toContain("stat -Lc '%u:%g:%a:%F' /proc/self/fd/9")
