...
**Implementation:** Keep the reviewed validator image/policy for tests only. Build and verify a dedicated platform-owned static-runtime child from the exact cached Node 22 slim base with `--pull=false`; set the non-root user, owned workdir, fixed helpers/entrypoint, no volumes, and minimal environment in the recipe. Build each validated application artifact into a second content-addressed image from the exact static-runtime image ID using `--network=none --pull=false` and a context containing only the generated HTML plus trusted recipe. Implement the generic runtime with atomic receipt primitives and a minimal reviewed Docker CLI environment. Serialize with an inter-process per-app lock. Never mount source or execute an app-authored server. Verify base image, static-runtime child, artifact image, and full created-container policy before start. Reconcile lazily on runtime/preview reads and explicitly during launcher readiness.
