...
`WO-MAO-047_THROUGH_WO-MAO-055_COMPLETE / WO-MAO-056_AND_WO-MAO-057_READY /
...
the malicious/defective worker drill; WO-MAO-051 is `COMPLETE` through the status/evidence/
owner-decision UX proof; WO-MAO-052 is `COMPLETE` through the kill/revoke/rollback/incident
...
concurrent certification lane execution; WO-MAO-056 and WO-MAO-057 are `READY`. That
...
mandatory sequence is now governed by the executable dependency graph: WO-MAO-056 and WO-MAO-057
are the next ready gates. The rejected local runtime is not in the eligible dependency chain.
