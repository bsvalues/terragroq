...
Status: `ACTIVE / PHASE_3 SCHEDULER MODEL CHECKING COMPLETE / WO-MAO-030 READY`
...
checking without provider or runtime execution. `WO-MAO-033` is `DEFERRED / PROVIDER_UNAVAILABLE` and
resumable. The only dependency-cleared Work Order is `WO-MAO-030`; later nodes remain pending on their
