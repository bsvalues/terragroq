# WO-AEH-039 — Seventy-two-hour limited pilot
...
```text
WORK_ORDER: WO-AEH-039
TITLE: Seventy-two-hour limited pilot
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-039 without expanding authority or scope.
...
OBJECTIVE: Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict.
SCOPE: Seventy-two-hour limited pilot; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict.
...
ACCEPTANCE_CRITERIA: Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: 72 continuous hours; useful bounded canary workload; controlled coordinator restart; zero prohibited events; complete terminal receipts and independent verdict; prove exact declared outcome: Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict.
VALIDATION_REQUIRED: 72 continuous hours; useful bounded canary workload; controlled coordinator restart; zero prohibited events; complete terminal receipts and independent verdict; prove exact declared outcome: Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-039-seventy-two-hour-limited-pilot.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-040) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict."
requiredValidation:
  - "72 continuous hours"
  - "useful bounded canary workload"
  - "controlled coordinator restart"
  - "zero prohibited events"
  - "complete terminal receipts and independent verdict"
  - "prove exact declared outcome: Run 72 continuous hours with useful canary work, at least one coordinator restart and zero prohibited events; issue an independent pass/reject verdict."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-039-seventy-two-hour-limited-pilot.md"
