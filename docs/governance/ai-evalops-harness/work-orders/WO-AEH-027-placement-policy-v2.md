# WO-AEH-027 — Placement policy v2
...
```text
WORK_ORDER: WO-AEH-027
TITLE: Placement policy v2
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-027 without expanding authority or scope.
...
OBJECTIVE: Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass.
SCOPE: Placement policy v2; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass.
...
ACCEPTANCE_CRITERIA: Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-027-placement-policy-v2.md
...
requiredOutputs:
  - "Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Rank only admitted nodes; freshness, capability, authority and capacity remain separate; saturation and partition simulations pass."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-027-placement-policy-v2.md"
