...
```text
WORK_ORDER: WO-AEH-018
TITLE: Restart-safe settlement descriptor
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-018 without expanding authority or scope.
...
OBJECTIVE: Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority.
SCOPE: Restart-safe settlement descriptor; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority.
...
ACCEPTANCE_CRITERIA: Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: reconstruct settlement after coordinator process death; reject tampered expired wrong-holder wrong-boot and stale-fence descriptors; prove repeated settlement is idempotent; prove exact declared outcome: Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority.
VALIDATION_REQUIRED: reconstruct settlement after coordinator process death; reject tampered expired wrong-holder wrong-boot and stale-fence descriptors; prove repeated settlement is idempotent; prove exact declared outcome: Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority.
...
ROLLBACK_OR_REVERSAL: Revert owned code and use expand-contract schema reversal before writes; after writes preserve descriptors and forward-fix while reconciling attempts.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-018-restart-safe-settlement-descriptor.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-019) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority."
requiredValidation:
  - "reconstruct settlement after coordinator process death"
  - "reject tampered expired wrong-holder wrong-boot and stale-fence descriptors"
  - "prove repeated settlement is idempotent"
  - "prove exact declared outcome: Persisted authenticated job/attempt/run/claim/lease/fence/holder/boot/authority/input/expiry binding replaces process-only settlement authority."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-018-restart-safe-settlement-descriptor.md"
