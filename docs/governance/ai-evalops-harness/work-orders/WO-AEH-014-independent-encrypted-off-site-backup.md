# WO-AEH-014 — Independent encrypted off-site backup
...
```text
WORK_ORDER: WO-AEH-014
TITLE: Independent encrypted off-site backup
...
RISK_CLASS: R3
DEPENDS_ON: WO-AEH-012, WO-AEH-013
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-014 without expanding authority or scope.
...
OBJECTIVE: Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate.
SCOPE: Independent encrypted off-site backup; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate.
...
ACCEPTANCE_CRITERIA: Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: encryption and immutable retention are enforced; provider credentials are independently scoped; replication freshness is under 24 hours; disposable recovery from the offsite copy passes; prove exact declared outcome: Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate.
VALIDATION_REQUIRED: encryption and immutable retention are enforced; provider credentials are independently scoped; replication freshness is under 24 hours; disposable recovery from the offsite copy passes; prove exact declared outcome: Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate.
...
ROLLBACK_OR_REVERSAL: Disable replication and revoke the scoped provider credential; preserve both local verified and remote immutable copies.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-014-independent-encrypted-off-site-backup.md
...
Environments:
- `AEGIS-live-backup-boundary`
- `approved-offsite-provider`
- `disposable-offsite-recovery`
...
requiredOutputs:
  - "Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate."
requiredValidation:
  - "encryption and immutable retention are enforced"
  - "provider credentials are independently scoped"
  - "replication freshness is under 24 hours"
  - "disposable recovery from the offsite copy passes"
  - "prove exact declared outcome: Separate credentials, encryption, immutability, daily replication and successful recovery from off-site copy. Provider/spend is an explicit decision gate."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-014-independent-encrypted-off-site-backup.md"
