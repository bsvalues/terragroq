# WO-AEH-037 — Node, power and network capacity qualification
...
```text
WORK_ORDER: WO-AEH-037
TITLE: Node, power and network capacity qualification
...
RISK_CLASS: R3
DEPENDS_ON: WO-AEH-044, WO-AEH-045
REPO: terragroq, HermesLab
BASE: terragroq=13709f5789c25dea408283730a6bd35e8fd894ab (review anchor; refresh at activation); HermesLab=0481061acf1f683688a00b09795647d0288c7232 (review anchor; refresh at activation)
BRANCH: not assigned; draft packet

PURPOSE: Deliver exactly the outcome named by WO-AEH-037 without expanding authority or scope.
...
OBJECTIVE: Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated.
SCOPE: Node, power and network capacity qualification; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
ALLOWED_FILES_OR_AREAS: terragroq:scripts; terragroq:config; terragroq:tests; terragroq:docs; HermesLab:hermes; HermesLab:atlas; HermesLab:aegis; terragroq:docs/reports/ai-evalops-harness
FILES_ALLOWED: terragroq:scripts; terragroq:config; terragroq:tests; terragroq:docs; HermesLab:hermes; HermesLab:atlas; HermesLab:aegis; terragroq:docs/reports/ai-evalops-harness
...
DELIVERABLES: Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated.

AUTHORITY_LEVEL: R3 / exact-scope authority required
AUTHORITY_GRANT: none
AUTHORITY_DECISION_ID: not assigned
AUTHORITY_GRANT_REF: not assigned
AUTHORITY_STATUS_EVENT_REFS: none
AUTHORITY_SUBJECT: WO-AEH-037
AUTHORITY_SCOPE_REQUIRED: Exact live hosts, actions, protected resources, and time window; separate activation authority is mandatory.
...
ACCEPTANCE_CRITERIA: Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: sustained thermal and throttle limits; UPS graceful shutdown proof; SMART and NVMe wear capture; iperf latency loss and throughput; backup and index contention; safe abort thresholds and explicit RAM GPU LAN upgrade thresholds; prove exact declared outcome: Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated.
VALIDATION_REQUIRED: sustained thermal and throttle limits; UPS graceful shutdown proof; SMART and NVMe wear capture; iperf latency loss and throughput; backup and index contention; safe abort thresholds and explicit RAM GPU LAN upgrade thresholds; prove exact declared outcome: Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated.
...
ROLLBACK_OR_REVERSAL: Stop load and fault tests immediately, restore prior service and network state, and verify node and authoritative-state health.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-037-node-power-and-network-capacity-qualification.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-036) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
Contracts:
- `wo-aeh-037-outcome`
- `Hermes-model-resource-envelope`
- `AEGIS-non-root-contained-worker`

Environments:
- `Hermes-live-capacity-test`
- `Atlas-live-protected-capacity-test`
- `AEGIS-live-capacity-test`
- `LAN-and-UPS-preauthorized-test`

Protected resources:
- `historical-evidence`
- `issue-357-quarantine`
- `Atlas-authoritative-state`
- `Hermes-inference-availability`
- `AEGIS-backup-and-worker-boundary`
- `Hermes-GPU-headroom`
- `model-digest-and-allowlist`
- `AEGIS-root-boundary`
- `AEGIS-backup-mounts`
...
authorityScopeRequired: "Exact live hosts, actions, protected resources, and time window; separate activation authority is mandatory."
requiredOutputs:
  - "Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated."
requiredValidation:
  - "sustained thermal and throttle limits"
  - "UPS graceful shutdown proof"
  - "SMART and NVMe wear capture"
  - "iperf latency loss and throughput"
  - "backup and index contention"
  - "safe abort thresholds and explicit RAM GPU LAN upgrade thresholds"
  - "prove exact declared outcome: Run sustained CPU/GPU thermal and throttle tests, power/UPS shutdown proof, SMART/NVMe wear trends, iperf latency/loss/throughput and backup/index contention; produce explicit RAM/GPU/LAN upgrade thresholds. Purchases remain separately owner-gated."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-037-node-power-and-network-capacity-qualification.md"
