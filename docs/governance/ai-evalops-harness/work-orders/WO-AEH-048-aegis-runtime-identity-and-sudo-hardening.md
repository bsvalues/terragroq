# WO-AEH-048 — AEGIS runtime identity and sudo hardening
...
```text
WORK_ORDER: WO-AEH-048
TITLE: AEGIS runtime identity and sudo hardening
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-048 without expanding authority or scope.
...
OBJECTIVE: Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access.
SCOPE: AEGIS runtime identity and sudo hardening; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access.
...
ACCEPTANCE_CRITERIA: Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; cgroup, process-tree, network, scratch, timeout, and cleanup tests; prove exact declared outcome: Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; cgroup, process-tree, network, scratch, timeout, and cleanup tests; prove exact declared outcome: Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-048-aegis-runtime-identity-and-sudo-hardening.md
...
requiredOutputs:
  - "Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access."
requiredValidation:
  ...
  - "prove exact declared outcome: Separate deploy/runtime identities, replace `NOPASSWD:ALL` with exact commands, prohibit interactive escalation and retain audited rollback access."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-048-aegis-runtime-identity-and-sudo-hardening.md"
