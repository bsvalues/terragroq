# WO-AEH-028 — One end-to-end AI lane
...
```text
WORK_ORDER: WO-AEH-028
TITLE: One end-to-end AI lane
...
RISK_CLASS: R3
DEPENDS_ON: WO-AEH-022, WO-AEH-023, WO-AEH-027
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-028 without expanding authority or scope.
...
OBJECTIVE: Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement.
SCOPE: One end-to-end AI lane; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement.
...
ACCEPTANCE_CRITERIA: Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: authenticated request to durable job to Hermes pull and admission to generation; signed receipt replicated off worker; restart-safe settlement after worker and coordinator-process restart; zero duplicate effects; prove exact declared outcome: Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement.
VALIDATION_REQUIRED: authenticated request to durable job to Hermes pull and admission to generation; signed receipt replicated off worker; restart-safe settlement after worker and coordinator-process restart; zero duplicate effects; prove exact declared outcome: Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement.
...
ROLLBACK_OR_REVERSAL: Engage kill switch, stop admission, fence workers, reconcile ambiguous attempts, restore last-known-good service and model, and forward-fix durable state.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-028-one-end-to-end-ai-lane.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-029, WO-AEH-036) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-029, WO-AEH-036
...
Environments:
- `Atlas-disposable-control-database`
- `Hermes-live-bounded-test`
...
requiredOutputs:
  - "Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement."
requiredValidation:
  - "authenticated request to durable job to Hermes pull and admission to generation"
  - "signed receipt replicated off worker"
  - "restart-safe settlement after worker and coordinator-process restart"
  - "zero duplicate effects"
  - "prove exact declared outcome: Authenticated request through durable job, Hermes pull, model admission, bounded generation, receipt, evidence replication and restart-safe settlement."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-028-one-end-to-end-ai-lane.md"
