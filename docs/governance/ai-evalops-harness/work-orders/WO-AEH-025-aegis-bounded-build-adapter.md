# WO-AEH-025 — AEGIS bounded build adapter
...
```text
WORK_ORDER: WO-AEH-025
TITLE: AEGIS bounded build adapter
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-025 without expanding authority or scope.
...
OBJECTIVE: Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup.
SCOPE: AEGIS bounded build adapter; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup.
...
ACCEPTANCE_CRITERIA: Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; cgroup, process-tree, network, scratch, timeout, and cleanup tests; prove exact declared outcome: Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; cgroup, process-tree, network, scratch, timeout, and cleanup tests; prove exact declared outcome: Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-025-aegis-bounded-build-adapter.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-026) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup."
requiredValidation:
  ...
  - "prove exact declared outcome: Content-addressed read-only input, isolated scratch, declared build outputs, explicit dependency/network policy, evidence and cancellation cleanup."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-025-aegis-bounded-build-adapter.md"
