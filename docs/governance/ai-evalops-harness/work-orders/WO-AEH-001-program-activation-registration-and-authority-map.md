...
```text
WORK_ORDER: WO-AEH-001
TITLE: Program activation, registration and authority map
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-001 without expanding authority or scope.
...
OBJECTIVE: After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`.
SCOPE: Program activation, registration and authority map; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
ALLOWED_FILES_OR_AREAS: terragroq:docs/governance; terragroq:docs/reports/ai-evalops-harness
FILES_ALLOWED: terragroq:docs/governance; terragroq:docs/reports/ai-evalops-harness
...
DELIVERABLES: After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`.
...
ACCEPTANCE_CRITERIA: After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: refresh and record both checkout bases; verify exact activation grant freshness and scope; validate the complete DAG and registry consistency; confirm issue #357 remains quarantined; prove exact declared outcome: After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`.
VALIDATION_REQUIRED: refresh and record both checkout bases; verify exact activation grant freshness and scope; validate the complete DAG and registry consistency; confirm issue #357 remains quarantined; prove exact declared outcome: After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`.
...
ROLLBACK_OR_REVERSAL: Revert only owned reviewed repository changes; preserve evidence and foreign or dirty state; rerun validation.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-001-program-activation-registration-and-authority-map.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-002, WO-AEH-011, WO-AEH-013) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-002, WO-AEH-011, WO-AEH-013
...
requiredOutputs:
  - "After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`."
requiredValidation:
  - "refresh and record both checkout bases"
  - "verify exact activation grant freshness and scope"
  - "validate the complete DAG and registry consistency"
  - "confirm issue #357 remains quarantined"
  - "prove exact declared outcome: After a separate matching activation grant exists, register the exact program, repositories, nodes, authority classes, blocked actions, reservations, evidence paths, owner counters and stop conditions; #357 quarantine preserved. Without that grant remain `BLOCKED_AUTHORITY`."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-001-program-activation-registration-and-authority-map.md"
