# WO-AEH-003 — Canonical maturity/status projection
...
```text
WORK_ORDER: WO-AEH-003
TITLE: Canonical maturity/status projection
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-003 without expanding authority or scope.
...
OBJECTIVE: One generated current-status view using the six maturity states; historical evidence remains immutable.
SCOPE: Canonical maturity/status projection; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: One generated current-status view using the six maturity states; historical evidence remains immutable.
...
ACCEPTANCE_CRITERIA: One generated current-status view using the six maturity states; historical evidence remains immutable; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: One generated current-status view using the six maturity states; historical evidence remains immutable.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: One generated current-status view using the six maturity states; historical evidence remains immutable.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-003-canonical-maturity-status-projection.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-008, WO-AEH-010, WO-AEH-049, WO-AEH-015) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-008, WO-AEH-010, WO-AEH-049, WO-AEH-015
...
requiredOutputs:
  - "One generated current-status view using the six maturity states; historical evidence remains immutable."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: One generated current-status view using the six maturity states; historical evidence remains immutable."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-003-canonical-maturity-status-projection.md"
