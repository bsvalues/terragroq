# WO-AEH-007 — Hermes state-service containment
...
```text
WORK_ORDER: WO-AEH-007
TITLE: Hermes state-service containment
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-007 without expanding authority or scope.
...
OBJECTIVE: Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion.
SCOPE: Hermes state-service containment; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
ALLOWED_FILES_OR_AREAS: HermesLab:hermes; HermesLab:SERVICE-MAP.md; terragroq:docs/reports/ai-evalops-harness
FILES_ALLOWED: HermesLab:hermes; HermesLab:SERVICE-MAP.md; terragroq:docs/reports/ai-evalops-harness
...
DELIVERABLES: Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion.
...
ACCEPTANCE_CRITERIA: Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: authorized clients remain functional; prohibited endpoints and remote sources are denied; no state is deleted; timed and console rollback succeeds; prove exact declared outcome: Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion.
VALIDATION_REQUIRED: authorized clients remain functional; prohibited endpoints and remote sources are denied; no state is deleted; timed and console rollback succeeds; prove exact declared outcome: Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion.
...
ROLLBACK_OR_REVERSAL: Restore the retained prior Compose, service, and network bindings; preserve all state; verify authorized access and denied remote access.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-007-hermes-state-service-containment.md
...
Environments:
- `Hermes-live-preauthorized-containment-boundary`
...
requiredOutputs:
  - "Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion."
requiredValidation:
  - "authorized clients remain functional"
  - "prohibited endpoints and remote sources are denied"
  - "no state is deleted"
  - "timed and console rollback succeeds"
  - "prove exact declared outcome: Stop or isolate recovery-only PostgreSQL/Redis, remove accidental state authority, bind permitted recovery endpoints safely, and prove rollback without data deletion."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-007-hermes-state-service-containment.md"
