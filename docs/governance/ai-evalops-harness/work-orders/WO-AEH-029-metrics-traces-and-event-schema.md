# WO-AEH-029 — Metrics, traces and event schema
...
```text
WORK_ORDER: WO-AEH-029
TITLE: Metrics, traces and event schema
...
RISK_CLASS: R2
DEPENDS_ON: WO-AEH-019, WO-AEH-024, WO-AEH-028
REPO: terragroq
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-029 without expanding authority or scope.
...
OBJECTIVE: Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass.
SCOPE: Metrics, traces and event schema; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass.
...
ACCEPTANCE_CRITERIA: Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: correlation continuity across program work-order job attempt lease fence node boot model authority and digests; prompt completion secret and environment leakage negatives; backward-compatible event schema fixtures; prove exact declared outcome: Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass.
VALIDATION_REQUIRED: correlation continuity across program work-order job attempt lease fence node boot model authority and digests; prompt completion secret and environment leakage negatives; backward-compatible event schema fixtures; prove exact declared outcome: Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-029-metrics-traces-and-event-schema.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-030) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass."
requiredValidation:
  - "correlation continuity across program work-order job attempt lease fence node boot model authority and digests"
  - "prompt completion secret and environment leakage negatives"
  - "backward-compatible event schema fixtures"
  - "prove exact declared outcome: Correlation across program/WO/job/attempt/lease/fence/node/boot/model/commit/authority/input/output digests; payload leakage tests pass."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-029-metrics-traces-and-event-schema.md"
