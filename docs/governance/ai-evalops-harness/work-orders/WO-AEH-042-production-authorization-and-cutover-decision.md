# WO-AEH-042 — Production authorization and cutover decision
...
```text
WORK_ORDER: WO-AEH-042
TITLE: Production authorization and cutover decision
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-042 without expanding authority or scope.
...
OBJECTIVE: Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`.
SCOPE: Production authorization and cutover decision; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`.
...
ACCEPTANCE_CRITERIA: Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: verify exact owner grant subject scope freshness target and cutover window; verify final canary kill switch and rollback readiness; treat absent or denied grant as SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED; prove exact declared outcome: Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`.
VALIDATION_REQUIRED: verify exact owner grant subject scope freshness target and cutover window; verify final canary kill switch and rollback readiness; treat absent or denied grant as SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED; prove exact declared outcome: Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`.
...
ROLLBACK_OR_REVERSAL: Revoke activation, stop new claims, fence and drain workers, restore the last-known-good release, and preserve the certification record.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-042-production-authorization-and-cutover-decision.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (program terminal or certification transition) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: program terminal or certification transition
NEXT_ON_BLOCK: If the owner grant is absent or denied, complete as SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED; otherwise use the typed authority or validation blocker without mutation.
...
requiredOutputs:
  - "Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`."
requiredValidation:
  - "verify exact owner grant subject scope freshness target and cutover window"
  - "verify final canary kill switch and rollback readiness"
  - "treat absent or denied grant as SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED"
  - "prove exact declared outcome: Separate owner decision and exact activation grant. Certification cannot self-authorize cutover; absent authority, remain `SOAK_PROVEN / NOT_PRODUCTION_AUTHORIZED`."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-042-production-authorization-and-cutover-decision.md"
