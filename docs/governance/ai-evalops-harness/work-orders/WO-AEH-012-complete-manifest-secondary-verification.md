...
```text
WORK_ORDER: WO-AEH-012
TITLE: Complete-manifest secondary verification
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-012 without expanding authority or scope.
...
OBJECTIVE: Primary and secondary complete manifests match for every protected source, not only PostgreSQL.
SCOPE: Complete-manifest secondary verification; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Primary and secondary complete manifests match for every protected source, not only PostgreSQL.
...
ACCEPTANCE_CRITERIA: Primary and secondary complete manifests match for every protected source, not only PostgreSQL; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Primary and secondary complete manifests match for every protected source, not only PostgreSQL.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Primary and secondary complete manifests match for every protected source, not only PostgreSQL.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-012-complete-manifest-secondary-verification.md
...
requiredOutputs:
  - "Primary and secondary complete manifests match for every protected source, not only PostgreSQL."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Primary and secondary complete manifests match for every protected source, not only PostgreSQL."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-012-complete-manifest-secondary-verification.md"
