# WO-AEH-051 — Staging deployment and rollback rehearsal
...
```text
WORK_ORDER: WO-AEH-051
TITLE: Staging deployment and rollback rehearsal
...
RISK_CLASS: R3
DEPENDS_ON: WO-AEH-038, WO-AEH-044
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-051 without expanding authority or scope.
...
OBJECTIVE: Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules.
SCOPE: Staging deployment and rollback rehearsal; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules.
...
ACCEPTANCE_CRITERIA: Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; migration, concurrency, idempotency, and restart tests; prove exact declared outcome: Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; migration, concurrency, idempotency, and restart tests; prove exact declared outcome: Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-051-staging-deployment-and-rollback-rehearsal.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-033, WO-AEH-036) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-033, WO-AEH-036
...
requiredOutputs:
  - "Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules."
requiredValidation:
  ...
  - "prove exact declared outcome: Deploy the exact artifact to staging, run login/work-order/evidence/AI canaries, promote within staging, restore the last-known-good artifact and verify DB forward-fix rules."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-051-staging-deployment-and-rollback-rehearsal.md"
