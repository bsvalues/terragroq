...
```text
WORK_ORDER: WO-AEH-011
TITLE: PostgreSQL restore-verification code and fixtures
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-011 without expanding authority or scope.
...
OBJECTIVE: Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups.
SCOPE: PostgreSQL restore-verification code and fixtures; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups.
...
ACCEPTANCE_CRITERIA: Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: successful disposable restore; corrupt partial and empty restores fail; psql nonzero status fails the pipeline; expected database schema table row and application-query invariants pass; live backups remain untouched; prove exact declared outcome: Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups.
VALIDATION_REQUIRED: successful disposable restore; corrupt partial and empty restores fail; psql nonzero status fails the pipeline; expected database schema table row and application-query invariants pass; live backups remain untouched; prove exact declared outcome: Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups.
...
ROLLBACK_OR_REVERSAL: Revert only owned backup-verification code and fixtures; do not mutate or prune live backup generations.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-011-postgresql-restore-verification-code-and-fixtures.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-012, WO-AEH-043) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-012, WO-AEH-043
...
Environments:
- `disposable-PostgreSQL-restore-fixtures`
...
requiredOutputs:
  - "Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups."
requiredValidation:
  - "successful disposable restore"
  - "corrupt partial and empty restores fail"
  - "psql nonzero status fails the pipeline"
  - "expected database schema table row and application-query invariants pass"
  - "live backups remain untouched"
  - "prove exact declared outcome: Modify backup/restore automation to enforce `ON_ERROR_STOP=1`, bound exit status and expected DB/schema/table/row/application-query invariants; corrupt, partial and empty fixture restores fail without touching live backups."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-011-postgresql-restore-verification-code-and-fixtures.md"
