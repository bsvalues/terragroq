# WO-AEH-022 — Hermes resident worker
...
```text
WORK_ORDER: WO-AEH-022
TITLE: Hermes resident worker
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-022 without expanding authority or scope.
...
OBJECTIVE: Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner.
SCOPE: Hermes resident worker; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner.
...
ACCEPTANCE_CRITERIA: Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: non-elevated install and uninstall; concurrency equals one; exact Ollama model allowlist enforcement; arbitrary command rejection; surviving descendant termination; restart reconciliation; prove exact declared outcome: Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner.
VALIDATION_REQUIRED: non-elevated install and uninstall; concurrency equals one; exact Ollama model allowlist enforcement; arbitrary command rejection; surviving descendant termination; restart reconciliation; prove exact declared outcome: Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-022-hermes-resident-worker.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-028) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
Environments:
- `Hermes-live-preauthorized-worker-boundary`
...
requiredOutputs:
  - "Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner."
requiredValidation:
  - "non-elevated install and uninstall"
  - "concurrency equals one"
  - "exact Ollama model allowlist enforcement"
  - "arbitrary command rejection"
  - "surviving descendant termination"
  - "restart reconciliation"
  - "prove exact declared outcome: Non-elevated service, concurrency one, exact Ollama allowlist, process-tree cancellation and restart reconciliation; no command runner."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-022-hermes-resident-worker.md"
