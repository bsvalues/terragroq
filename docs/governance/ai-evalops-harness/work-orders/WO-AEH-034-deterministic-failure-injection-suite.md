# WO-AEH-034 — Deterministic failure-injection suite
...
```text
WORK_ORDER: WO-AEH-034
TITLE: Deterministic failure-injection suite
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-034 without expanding authority or scope.
...
OBJECTIVE: Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults.
SCOPE: Deterministic failure-injection suite; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults.
...
ACCEPTANCE_CRITERIA: Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; zero prohibited events and complete immutable evidence; prove exact declared outcome: Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; zero prohibited events and complete immutable evidence; prove exact declared outcome: Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-034-deterministic-failure-injection-suite.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-035) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults."
requiredValidation:
  ...
  - "prove exact declared outcome: Coordinator/worker death, descendant survival, OOM, full disk, fsync, stale fence, duplicates, clock, DB/network/provider and cancellation faults."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-034-deterministic-failure-injection-suite.md"
