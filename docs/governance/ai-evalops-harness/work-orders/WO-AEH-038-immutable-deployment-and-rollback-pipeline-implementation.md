# WO-AEH-038 — Immutable deployment and rollback pipeline implementation
...
```text
WORK_ORDER: WO-AEH-038
TITLE: Immutable deployment and rollback pipeline implementation
...
RISK_CLASS: R2
DEPENDS_ON: WO-AEH-008, WO-AEH-009, WO-AEH-010, WO-AEH-030
REPO: terragroq
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-038 without expanding authority or scope.
...
OBJECTIVE: Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment.
SCOPE: Immutable deployment and rollback pipeline implementation; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment.
...
ACCEPTANCE_CRITERIA: Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-038-immutable-deployment-and-rollback-pipeline-implementation.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-051) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Build once from locks; retain digest, SBOM, scan, provenance, migration plan, staging promotion, canary and last-known-good rollback automation; no live deployment."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-038-immutable-deployment-and-rollback-pipeline-implementation.md"
