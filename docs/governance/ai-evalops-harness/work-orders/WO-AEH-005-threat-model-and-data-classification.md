...
```text
WORK_ORDER: WO-AEH-005
TITLE: Threat model and data classification
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-005 without expanding authority or scope.
...
OBJECTIVE: Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements.
SCOPE: Threat model and data classification; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements.
...
ACCEPTANCE_CRITERIA: Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-005-threat-model-and-data-classification.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-006, WO-AEH-010) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-006, WO-AEH-010
...
requiredOutputs:
  - "Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Trust boundaries, threats, payload classifications, telemetry redaction and retention requirements."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-005-threat-model-and-data-classification.md"
