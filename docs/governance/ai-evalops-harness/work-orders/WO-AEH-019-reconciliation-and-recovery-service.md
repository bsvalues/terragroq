...
```text
WORK_ORDER: WO-AEH-019
TITLE: Reconciliation and recovery service
...
RISK_CLASS: R2
DEPENDS_ON: WO-AEH-017, WO-AEH-018
REPO: terragroq
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-019 without expanding authority or scope.
...
OBJECTIVE: Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes.
SCOPE: Reconciliation and recovery service; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes.
...
ACCEPTANCE_CRITERIA: Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; migration, concurrency, idempotency, and restart tests; prove exact declared outcome: Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; migration, concurrency, idempotency, and restart tests; prove exact declared outcome: Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-019-reconciliation-and-recovery-service.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-021, WO-AEH-029) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-021, WO-AEH-029
...
requiredOutputs:
  - "Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes."
requiredValidation:
  ...
  - "prove exact declared outcome: Idempotent settle/recover/reconcile and typed `NOT_EXECUTED`, `EXECUTED`, `AMBIGUOUS`, `EXPIRED`, `FENCED` outcomes."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-019-reconciliation-and-recovery-service.md"
