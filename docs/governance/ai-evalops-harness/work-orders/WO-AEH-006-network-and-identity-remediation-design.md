...
```text
WORK_ORDER: WO-AEH-006
TITLE: Network and identity remediation design
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-006 without expanding authority or scope.
...
OBJECTIVE: Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation.
SCOPE: Network and identity remediation design; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation.
...
ACCEPTANCE_CRITERIA: Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-006-network-and-identity-remediation-design.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-007, WO-AEH-046, WO-AEH-047, WO-AEH-048) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-007, WO-AEH-046, WO-AEH-047, WO-AEH-048
...
requiredOutputs:
  - "Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Exact target bindings, firewall allowlists, identities, sudo commands, TLS/auth and console-safe rollback. No mutation."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-006-network-and-identity-remediation-design.md"
