...
```text
WORK_ORDER: WO-AEH-023
TITLE: Model-aware GPU admission
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-023 without expanding authority or scope.
...
OBJECTIVE: Measured weights/runtime/KV/context envelope plus free VRAM, reserve, temperature, queue and residency admission.
SCOPE: Model-aware GPU admission; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Measured weights/runtime/KV/context envelope plus free VRAM, reserve, temperature, queue and residency admission.
...
ACCEPTANCE_CRITERIA: Measured weights/runtime/KV/context envelope plus free VRAM, reserve, temperature, queue and residency admission; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; TTFT, tokens-per-second, VRAM, thermal, OOM, and cancellation evidence; prove exact declared outcome: Measured weights/runtime/KV/context envelope plus free VRAM, reserve, temperature, queue and residency admission.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-023-model-aware-gpu-admission.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-026, WO-AEH-028) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-026, WO-AEH-028
...
requiredOutputs:
  - "Measured weights/runtime/KV/context envelope plus free VRAM, reserve, temperature, queue and residency admission."
requiredValidation:
  ...
  - "prove exact declared outcome: Measured weights/runtime/KV/context envelope plus free VRAM, reserve, temperature, queue and residency admission."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-023-model-aware-gpu-admission.md"
