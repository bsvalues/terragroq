# WO-AEH-015 — Durable schema implementation
...
```text
WORK_ORDER: WO-AEH-015
TITLE: Durable schema implementation
...
RISK_CLASS: R2
DEPENDS_ON: WO-AEH-003, WO-AEH-004, WO-AEH-009, WO-AEH-010
REPO: terragroq
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-015 without expanding authority or scope.
...
OBJECTIVE: Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences.
SCOPE: Durable schema implementation; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences.
...
ACCEPTANCE_CRITERIA: Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; migration, concurrency, idempotency, and restart tests; prove exact declared outcome: Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; migration, concurrency, idempotency, and restart tests; prove exact declared outcome: Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-015-durable-schema-implementation.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-016) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences."
requiredValidation:
  ...
  - "prove exact declared outcome: Jobs, attempts, leases, events, outbox, workers and evidence references with unique idempotency and monotonic fences."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-015-durable-schema-implementation.md"
