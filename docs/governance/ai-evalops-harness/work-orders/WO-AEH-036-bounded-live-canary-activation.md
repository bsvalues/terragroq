# WO-AEH-036 — Bounded live canary activation
...
```text
WORK_ORDER: WO-AEH-036
TITLE: Bounded live canary activation
...
RISK_CLASS: R3
DEPENDS_ON: WO-AEH-014, WO-AEH-028, WO-AEH-033, WO-AEH-035, WO-AEH-037, WO-AEH-043, WO-AEH-044, WO-AEH-051
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-036 without expanding authority or scope.
...
OBJECTIVE: Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry.
SCOPE: Bounded live canary activation; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry.
...
ACCEPTANCE_CRITERIA: Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; zero prohibited events and complete immutable evidence; prove exact declared outcome: Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; typed fail-closed and rollback verification; zero prohibited events and complete immutable evidence; prove exact declared outcome: Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry.
...
ROLLBACK_OR_REVERSAL: Disable admission; drain or fence active attempts; restore the exact last-known-good artifact, service unit, network policy, model, or configuration; reconcile ambiguous attempts; verify kill switch and retained evidence.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-036-bounded-live-canary-activation.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-039) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
requiredOutputs:
  - "Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "typed fail-closed and rollback verification"
  - "zero prohibited events and complete immutable evidence"
  - "prove exact declared outcome: Activate only one Hermes inference lane and the already reviewed AEGIS HASH_VERIFY class at concurrency one; prove kill switch, signed receipts and rollback before pilot entry."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-036-bounded-live-canary-activation.md"
