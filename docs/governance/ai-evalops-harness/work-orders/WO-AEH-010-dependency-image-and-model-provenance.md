# WO-AEH-010 — Dependency, image and model provenance
...
```text
WORK_ORDER: WO-AEH-010
TITLE: Dependency, image and model provenance
...
RISK_CLASS: R2
DEPENDS_ON: WO-AEH-003, WO-AEH-005
REPO: terragroq
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-010 without expanding authority or scope.
...
OBJECTIVE: Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment.
SCOPE: Dependency, image and model provenance; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment.
...
ACCEPTANCE_CRITERIA: Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-010-dependency-image-and-model-provenance.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-015, WO-AEH-038) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-015, WO-AEH-038
...
requiredOutputs:
  - "Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Modify and verify dependency locks/manifests, immutable image/model digests, SBOM, scan and last-known-good manifest; no live deployment."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-010-dependency-image-and-model-provenance.md"
