# WO-AEH-024 — AEGIS bounded worker
...
```text
WORK_ORDER: WO-AEH-024
TITLE: AEGIS bounded worker
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-024 without expanding authority or scope.
...
OBJECTIVE: Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first.
SCOPE: AEGIS bounded worker; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
DELIVERABLES: Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first.
...
ACCEPTANCE_CRITERIA: Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: non-root identity; HASH_VERIFY is the only initially admitted operation; cgroup CPU RAM scratch and time limits; arbitrary command and undeclared egress denial; install and uninstall proof; prove exact declared outcome: Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first.
VALIDATION_REQUIRED: non-root identity; HASH_VERIFY is the only initially admitted operation; cgroup CPU RAM scratch and time limits; arbitrary command and undeclared egress denial; install and uninstall proof; prove exact declared outcome: Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-024-aegis-bounded-worker.md

SUCCESS_TRANSITION: Record the verified result and release only dependency successors (WO-AEH-025, WO-AEH-050, WO-AEH-026, WO-AEH-029) to fresh dependency, reservation, and AUTHORITY_MATCH evaluation; success grants no authority.
...
NEXT_ON_PASS: WO-AEH-025, WO-AEH-050, WO-AEH-026, WO-AEH-029
...
Environments:
- `AEGIS-live-preauthorized-worker-boundary`
...
requiredOutputs:
  - "Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first."
requiredValidation:
  - "non-root identity"
  - "HASH_VERIFY is the only initially admitted operation"
  - "cgroup CPU RAM scratch and time limits"
  - "arbitrary command and undeclared egress denial"
  - "install and uninstall proof"
  - "prove exact declared outcome: Non-root systemd worker with CPU, memory, scratch, time, process and network containment; `HASH_VERIFY` first."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-024-aegis-bounded-worker.md"
