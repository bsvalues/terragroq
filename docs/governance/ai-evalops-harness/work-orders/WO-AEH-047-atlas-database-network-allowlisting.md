# WO-AEH-047 — Atlas database network allowlisting
...
```text
WORK_ORDER: WO-AEH-047
TITLE: Atlas database network allowlisting
...
PURPOSE: Deliver exactly the outcome named by WO-AEH-047 without expanding authority or scope.
...
OBJECTIVE: Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback.
SCOPE: Atlas database network allowlisting; only the repository-qualified areas, contracts, environments, and protected resources in this packet.
...
ALLOWED_FILES_OR_AREAS: HermesLab:atlas; HermesLab:SERVICE-MAP.md; terragroq:docs/reports/ai-evalops-harness
FILES_ALLOWED: HermesLab:atlas; HermesLab:SERVICE-MAP.md; terragroq:docs/reports/ai-evalops-harness
...
DELIVERABLES: Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback.
...
ACCEPTANCE_CRITERIA: Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback; every required validation below passes including fail-closed negatives; rollback is verified; evidence is immutable; and independent review has no blocking findings.
VALIDATION: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback.
VALIDATION_REQUIRED: focused unit and negative tests; changed-path and secret scan; git diff --check; independent review; prove exact declared outcome: Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback.
...
EVIDENCE_PATH: terragroq:docs/reports/ai-evalops-harness/WO-AEH-047-atlas-database-network-allowlisting.md
...
requiredOutputs:
  - "Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback."
requiredValidation:
  - "focused unit and negative tests"
  - "changed-path and secret scan"
  - "git diff --check"
  - "independent review"
  - "prove exact declared outcome: Permit only declared clients to PostgreSQL/Mongo/Redis; verify application/backup access and deny all other sources with timed rule rollback."
reviewRequirements:
...
stopConditions:
  ...
  - issue-357-reuse
evidenceTargets:
  - "terragroq:docs/reports/ai-evalops-harness/WO-AEH-047-atlas-database-network-allowlisting.md"
