...
const EVIDENCED_COMPLETE = new Set([
  ...
  ...(isVerifiedWoMao053ResilienceSafetyRollupEvidence(MULTI_AGENT_RESILIENCE_SAFETY_ROLLUP_EVIDENCE)
    ? [workOrderId(53)]
    : []),
  ...(isVerifiedWoMao054CertificationPortfolioEvidence(MULTI_AGENT_CERTIFICATION_PORTFOLIO_EVIDENCE)
    ? [workOrderId(54)]
    : []),
  ...(isVerifiedWoMao055ConcurrentCertificationEvidence(MULTI_AGENT_CONCURRENT_CERTIFICATION_EVIDENCE)
    ? [workOrderId(55)]
    : []),
  ...(isVerifiedWoMao056CrossReviewCiEvidence(MULTI_AGENT_CROSS_REVIEW_CI_EVIDENCE)
    ? [workOrderId(56)]
    : []),
  ...(isVerifiedWoMao057LiveFailureRecoveryEvidence(MULTI_AGENT_LIVE_FAILURE_RECOVERY_EVIDENCE)
    ? [workOrderId(57)]
    : []),
