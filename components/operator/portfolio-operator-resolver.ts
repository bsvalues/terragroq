...
export function buildWorkOrderChain(
  ...
  const titles = program.programId === "PROGRAM-WILLIAMOS-LOCAL-IDENTITY-RUNTIME-001"
    ...
    : ownerOutcomeDelivery
    ? OWNER_OUTCOME_PROGRAM_WORK_ORDERS.map(([, title]) => title)
  ...
  const ownerOutcomeStatuses = ownerOutcomeDelivery
    ? ownerOutcomeCompletedCount === undefined
      ? OWNER_OUTCOME_PROGRAM_WORK_ORDERS.map(([, , status]) => status)
  ...
  return titles.map((title, index) => ({
    ...
    validationPlan: ["Focused tests", "git diff --check", "lint", "full tests", "build"],
    evidence: ownerOutcomeDelivery
      ? OWNER_OUTCOME_PROGRAM_WORK_ORDERS[index][3]
      : `docs/reports/${workOrderPrefix}-${String(index + 1).padStart(3, "0")}.md`,
