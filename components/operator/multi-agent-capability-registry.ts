...
export const MULTI_AGENT_CAPABILITY_INVENTORY = Object.freeze([
  ...
  record({
    capabilityId: "kill-revoke-rollback-incident-procedure-model",
    label: "Kill, revoke, rollback, and incident procedure",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-052 proves static incident procedure decisions for worker/provider quarantine, checkpoint preservation, owned rollback, foreign cleanup denial, and genuine owner authority walls.",
    ...
    restrictions: [
      ...
      "Rollback is modeled only for owned reserved changes; foreign paths, dirty worktrees, and production state remain blocked",
      "Owner contact is represented only for genuine protected authority or credential gates",
      "WO-MAO-052 is complete; WO-MAO-053 is now READY through retained prerequisites",
      "No command runner, background worker, runtime activation, credential access, authority minting, GitHub execution, production write, or owner relay",
    ],
  }),
  record({
    capabilityId: "resilience-safety-rollup-model",
    label: "Resilience and safety rollup",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-053 proves the Phase 6 resilience and safety evidence chain across secret trust, retry/idempotency, recovery, failover, race handling, defective-worker containment, owner-decision UX, and incident procedure.",
    ...
    restrictions: [
      ...
      "Certification readiness is evidence-only and does not grant certification authority",
      "WO-MAO-053 is complete; WO-MAO-054 is now READY through retained prerequisites",
      "No command runner, background worker, runtime activation, credential access, authority minting, GitHub execution, production write, or owner relay",
    ],
  }),
  record({
    capabilityId: "certification-portfolio-selection-model",
    label: "Certification portfolio selection",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-054 selects two independent useful Codex certification lanes, one dependent fan-in lane, and excludes Claude with the existing provider-unavailable finding.",
    ...
    restrictions: [
      ...
      "Claude Code remains excluded unless a future conformant provider proof exists",
      "WO-MAO-054 is complete; WO-MAO-055 is now READY through retained prerequisites",
      "No command runner, background worker, runtime activation, credential access, authority minting, GitHub execution, production write, or owner relay",
    ],
  }),
  record({
    capabilityId: "concurrent-certification-lanes-model",
    label: "Concurrent certification lanes",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-055 executes two independent static useful Codex certification lanes with disjoint reservations and records the dependent fan-in projection.",
    ...
    restrictions: [
      ...
      "Claude Code remains excluded by the typed provider-unavailable finding",
      "WO-MAO-055 is complete; WO-MAO-056 and WO-MAO-057 are now READY through retained prerequisites",
      "No command runner, background worker, runtime activation, credential access, authority minting, GitHub execution, production write, or owner relay",
    ],
  }),
  record({
    capabilityId: "cross-review-ci-remediation-certification-model",
    label: "Cross-review and CI remediation certification",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-056 records one requested-changes cycle, one classified CI repair cycle, zero unresolved review threads, and corrected WO-MAO-055 reservation accounting.",
    ...
    restrictions: [
      ...
      "Review and CI remediation are certification claims, not new authority grants",
      "WO-MAO-056 is complete; WO-MAO-057 remains READY and WO-MAO-058 waits for WO-MAO-057",
      "No command runner, background worker, runtime activation, credential access, authority minting, GitHub execution, production write, or owner relay",
    ],
  }),
  record({
    capabilityId: "live-failure-recovery-certification-model",
    label: "Live failure and recovery certification",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-057 records five bounded live recovery injections during useful repository delivery: worker death, coordinator restart, provider/network failure, reservation collision, and stale-base refresh.",
    ...
    restrictions: [
      ...
      "The stale-base proof used PR #411, then refreshed the certification branch and requires full revalidation before merge",
  ...
  record({
    capabilityId: "merge-verify-clean-fanin-release-model",
    label: "Merge, verification, cleanup, and fan-in release certification",
    kind: "CONTROL_PLANE",
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-058 records the merged PR #411 and PR #412 fan-in, main/prod verification, owned disposable branch cleanup classification, and release of WO-MAO-059.",
    ...
    restrictions: [
      ...
      "Cleanup is limited to owned disposable lane resources and denies .obsidian, shared worktree, foreign branch, unmerged branch, evidence artifact, and runtime state cleanup",
      "The certification model remains non-executable and does not activate a runtime, scheduler, worker, command runner, or provider dispatch path",
