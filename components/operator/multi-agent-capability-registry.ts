...
export const MULTI_AGENT_CAPABILITY_INVENTORY = Object.freeze([
  ...
  record({
    ...
    executionClass: "WORKER_CANDIDATE",
    coordinationEligible: true,
    claim: "WO-MAO-030 translates selected hosted team plans into bounded current-session native assignments, messages, cancellations, and sanitized evidence without durable dispatch.",
  ...
  record({
    ...
    executionClass: "WORKER_CANDIDATE",
    coordinationEligible: true,
    claim: "WO-MAO-031 proves current-session builder, independent assurance, original-builder remediation, and bounded re-review adapters using the common provider contract without durable dispatch.",
    reasonCode: "HOSTED_CODEX_ROLE_ADAPTERS_PROVEN",
  ...
  record({
    ...
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-034 proves static provider routing and review planning that excludes unavailable providers, preserves same-provider independent review, and grants no dispatch authority.",
  ...
  record({
    ...
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-035 proves static provider failure classification, circuit breaker/backoff decisions, and bounded fallback selection without provider calls or dispatch.",
  ...
  record({
    ...
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-036 proves suite-level provider conformance classification without certifying disabled, unavailable, rejected, or non-service session-only providers as executable workers.",
  ...
  record({
    ...
    status: "PROVEN",
    executionClass: "NON_EXECUTABLE",
    claim: "WO-MAO-037 proves branch, commit, and push gates for active authority, preventive trust, reserved-file-only staging, foreign-change exclusion, exact secret scan coverage, attributable identity, and rollback preservation.",
    ...
    restrictions: [
      ...
      "No background Git worker, service transport, or durable runtime",
