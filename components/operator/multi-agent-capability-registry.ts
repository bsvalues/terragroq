...
export const MULTI_AGENT_CAPABILITY_INVENTORY = Object.freeze([
  ...
  record({
    ...
    claim: "A hosted Codex session executed the bounded Phase 1 R1 lanes, but two post-merge findings keep final coordination assurance pending.",
    reasonCode: "HOSTED_COORDINATION_REMEDIATION_REVIEW_PENDING",
    adapterRef: null,
    ...
    restrictions: [
      "Bounded R0/R1 repository coordination only",
      "Final proof pending independent remediation re-review",
  ...
  record({
    ...
    executionClass: "WORKER_CANDIDATE",
    coordinationEligible: true,
    claim: "Native coordinator, builders, assurance, remediation, and fan-in roles executed, but post-merge findings prevent a final bounded-proof verdict.",
    reasonCode: "CODEX_NATIVE_TEAM_REMEDIATION_REVIEW_PENDING",
    adapterRef: null,
    authorityGrantRefs: ["docs/governance/active-program-queue.md#canonical-active-program"],
    trustGateRef: PREVENTIVE_TRUST_GATE_V2_REF,
    evidence: [
