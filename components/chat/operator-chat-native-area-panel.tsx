...
export function OperatorChatNativeAreaPanel() {
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card">
      <div className="border-b border-border bg-muted/30 px-4 py-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <StatusBadge value="requires_approval" label="authority gated" />
          <StatusBadge value="inactive" label="runtime unchanged" />
        </div>
        ...
        {area.sections.map((section) => (
          <article key={section.label} className="rounded-lg border border-border bg-card p-3">
            ...
            <dl className="mt-3 grid gap-2 text-xs text-muted-foreground">
              <OperatorChatField label="Boundary" value={section.boundary} />
              <OperatorChatField label="Evidence" value={section.evidenceLinkage} />
            </dl>
      ...
      <div className="flex items-start gap-2 px-4 py-3">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          Operator Chat may inspect, prepare, route, review, draft next moves, and shape safe
          handoffs. This reframe does not change chat runtime, model provider, message
          persistence, memory writes, retrieval, tool execution, Work Order execution, auth,
          access grants, DB schema, env, packages, Vercel settings, Hermes, MCP, autonomy,
          or production-write behavior.
        </p>
      </div>
    </section>
  )
...
function OperatorChatField({ label, value }: { label: string; value: string }) {
  return (
    <div>
