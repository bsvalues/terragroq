...
export function CorpusNativeAreaPanel() {
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card">
      <div className="border-b border-border bg-muted/30 px-4 py-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <StatusBadge value="requires_approval" label="review required" />
          <StatusBadge value="inactive" label="retrieval unchanged" />
        </div>
        ...
        {area.sections.map((section) => (
          <article key={section.label} className="rounded-lg border border-border bg-card p-3">
            ...
            <dl className="mt-3 grid gap-2 text-xs text-muted-foreground">
              <CorpusField label="Provenance" value={section.provenance} />
              <CorpusField label="Authority" value={section.authorityRequirement} />
              <CorpusField label="Evidence" value={section.evidenceLinkage} />
            </dl>
      ...
      <div className="flex items-start gap-2 px-4 py-3">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          Corpus may organize source material, provenance, citations, and evidence-linked
          context. This reframe does not change ingestion, upload behavior, embedding,
          vector retrieval, external connectors, corpus mutation behavior, auth, access
          grants, token handling, audit writers, durable limiters, runtime validation,
          permission models, DB schema, Hermes, MCP, autonomy, or production-write behavior.
        </p>
      </div>
    </section>
  )
...
function CorpusField({ label, value }: { label: string; value: string }) {
  return (
    <div>
