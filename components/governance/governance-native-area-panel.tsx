...
export function GovernanceNativeAreaPanel() {
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card">
        ...
        {area.sections.map((section) => (
          <article key={section.label} className="rounded-lg border border-border bg-card p-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <StatusBadge value="neutral" label={section.posture} />
            </div>
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              {section.purpose}
            </p>
            <dl className="mt-3 grid gap-2 text-xs text-muted-foreground">
              <GovernanceField label="Allowed" value={section.allowed} />
              <GovernanceField label="Blocked" value={section.blocked} />
              <GovernanceField label="Approval" value={section.ownerApproval} />
            </dl>
      ...
      <div className="flex items-start gap-2 px-4 py-3">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          Governance may show gates, blocked decisions, and access posture. This reframe
          does not change auth behavior, auth policy, access grants, token handling, audit
          writers, durable limiters, runtime validation, permission models, approval
          execution, automatic approval, DB schema, Hermes, MCP, autonomy, or production-write behavior.
        </p>
      </div>
    </section>
  )
...
function GovernanceField({ label, value }: { label: string; value: string }) {
  return (
    <div>
