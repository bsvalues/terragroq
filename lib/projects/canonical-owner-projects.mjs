...
export function buildWilliamOsOwnerProjectPlan(userId) {
  const canonical = buildCanonicalOwnerProjectPlan(userId)
  return {
    userId: canonical.userId,
    projects: canonical.projects.filter((row) => row.key === "williamos"),
    resources: canonical.resources.filter((row) => row.projectKey === "williamos"),
  }
}
