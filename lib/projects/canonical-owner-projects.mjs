...
export async function applyCanonicalOwnerProjectPlan({ client, plan }) {
  ...
  try {
    ...
    for (const row of plan.resources) {
      ...
      const primaryRepos = await client.query(
        'select "canonicalIdentity" from "project_resource" where "userId" = $1 and "projectId" = $2 and "type" = $3 and "relationship" = $4',
        [plan.userId, projectId, row.type, row.relationship],
      )
      if (primaryRepos.rowCount !== 1 || primaryRepos.rows[0]?.canonicalIdentity !== row.canonicalIdentity) {
        throw new Error(`PROJECT_BACKFILL_PRIMARY_REPO_CONFLICT:${row.projectKey}`)
      }
    }
