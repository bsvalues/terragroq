...
export async function runGovernedCreateTransaction(transaction) {
  const {
    ...
    publishCreationIntent,
    bindCreationCandidate,
    bindCreationPublication,
    releaseCreationIntent,
    deactivateCreationIntent,
  } = transaction
  const code = typeof errorPrefix === "string" ? errorPrefix : "APPLICATION_PROPOSAL"
  const checkpoint = async (stage, state = {}) => transaction.transactionOperations?.checkpoint?.(stage, {
    proposalId, branch, workspace, ...state,
  })
  ...
  let creationCleanupProven = true
  const ownedBranchTargets = new Set()
  const temporaryIndex = `${workspace}.candidate-index-${transaction.creationIntentToken ?? crypto.randomUUID()}`
  const temporaryIndexLock = `${temporaryIndex}.lock`
  try {
    creationIntent = await publishCreationIntent?.({ baseRef, baseSha, createdAt }) ?? null
    await checkpoint("intent_published", { baseSha })
    ...
    await checkpoint("worktree_created", { baseSha })
    await assertRepositoryBoundary?.()
    ...
    await assertRepositoryBoundary?.()
    await prepareCandidateIndex?.(temporaryIndex, temporaryIndexLock)
    await checkpoint("candidate_index_prepared", { candidateIndex: temporaryIndex })
    const indexEnvironment = gitIndexEnvironment(temporaryIndex)
    await git(workspace, ["read-tree", baseSha], { env: indexEnvironment })
    await assertCandidateIndex?.(temporaryIndex, temporaryIndexLock)
    ...
    assertSnapshot(workspace, proposed, `${code}_VALIDATION_HASH_MISMATCH`)
    if (creationIntent) creationIntent = await bindCreationCandidate?.(creationIntent, candidateSha) ?? creationIntent
    await checkpoint("candidate_bound", { baseSha, candidateSha })
    await assertRepositoryBoundary?.()
    await assertReferenceBoundary?.(`refs/heads/${branch}`)
    await git(repository, ["update-ref", `refs/heads/${branch}`, candidateSha, baseSha])
    await checkpoint("candidate_published", { baseSha, candidateSha })
    ...
    await checkpoint("worktree_removed", { baseSha, candidateSha })
    ...
    if (creationIntent) {
      creationIntent = await bindCreationPublication?.(creationIntent, {
        candidateSha, changedPaths: paths, patchBytes: bytes, receipt: value,
      }) ?? creationIntent
    }
    await checkpoint("publication_bound", { baseSha, candidateSha })
    ...
    try {
      if (cleanupCandidateIndex) await cleanupCandidateIndex(temporaryIndex, temporaryIndexLock)
      else {
        fs.rmSync(temporaryIndexLock, { force: true })
    ...
    let intentReleaseFailure = null
    if (creationIntent && !cleanupFailure && creationCleanupProven) {
      try { await releaseCreationIntent?.(creationIntent, { persisted }) }
      catch (error) {
        // A durable receipt is authoritative; its orphaned intent can be
        // reconciled on the next bounded recovery pass without inviting a
        // duplicate proposal retry.
        if (!persisted) intentReleaseFailure = error
      }
    }
    try { if (creationIntent) await deactivateCreationIntent?.(creationIntent) } catch { /* liveness is in-process only */ }
    if (cleanupFailure) {
      throw new Error(transaction.worktreeCleanupErrorCode ?? `${code}_ARTIFACT_CLEANUP_FAILED`)
    }
    if (intentReleaseFailure) throw intentReleaseFailure
  }
}
