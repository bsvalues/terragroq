...
export async function createApplicationProposal({
  ...
  const workspace = applicationProposalWorkspace(runtimeRoot, `${descriptor.manifest.id}-${proposalId}`)
  ...
  return runGovernedCreateTransaction({
    ...
    prepareStorage: async () => {
      ...
      if ([files.receipt, files.patch, files.quarantine].some((target) => fs.existsSync(target))) {
        throw new Error("APPLICATION_PROPOSAL_RECEIPT_INVALID")
      }
    },
    writePatch: async (_files, bytes) => {
      fs.writeFileSync(stagedPatch, bytes, { flag: "wx" })
      const descriptor = fs.openSync(stagedPatch, "r+")
      try { fs.fsyncSync(descriptor) } finally { fs.closeSync(descriptor) }
      return stagedPatch
    },
    publishPatch: async () => { fs.renameSync(stagedPatch, files.patch) },
    writeReceipt: async (_files, value) => { writeReceipt(files.receipt, value, runtimeRoot) },
    cleanupCreationArtifacts: async ({ bytes }) => {
      let clean = true
      try { fs.rmSync(stagedPatch, { force: true }) } catch { clean = false }
      if (fs.existsSync(files.patch)) {
        try {
          const owned = fs.readFileSync(files.patch)
          if (!Buffer.isBuffer(bytes) || digest(owned) !== digest(bytes)) throw new Error()
          fs.unlinkSync(files.patch)
          if (fs.existsSync(files.patch)) throw new Error()
        } catch { clean = false }
      }
...
async function applyTransaction({ descriptor, runtimeRoot, stored, claim, validateWorkspace, transactionOperations = {} }) {
  ...
  try {
    ...
    validationResult,
    removeWorktree,
    createAppliedReceipt: ({ validation }) => validReceipt({
      ...value,
      status: "APPLIED",
      validation,
      appliedAt: new Date().toISOString(),
