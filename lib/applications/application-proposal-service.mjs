...
function bootstrapProposalListIndex(runtimeRoot, applicationId) {
  return bootstrapApplicationProposalListIndex({
    runtimeRoot,
    applicationId,
    describeProposal: (proposalId) => ({ createdAt: receipt(runtimeRoot, applicationId, proposalId).value.createdAt }),
  })
}

function indexDurableCreateArtifact(runtimeRoot, applicationId, proposalId, operationToken) {
  let stored
  try { stored = receipt(runtimeRoot, applicationId, proposalId) }
  catch (error) {
    if (error?.message === "APPLICATION_PROPOSAL_NOT_FOUND") return false
    throw error
  }
  addApplicationProposalToListIndex({
    runtimeRoot,
...
async function removeWorktree(repository, runtimeRoot, workspace) {
  ...
  await git(repository, ["worktree", "remove", "--force", workspace], { allowFailure: true })
...
async function recoverDeadApplicationCreateIntent(descriptor, runtimeRoot, handle) {
  ...
  if (hasReceiptArtifact) {
    ...
    indexDurableCreateArtifact(runtimeRoot, intent.applicationId, intent.proposalId, intent.intentToken)
...
async function reconcileApplicationCreateIntents(descriptor, runtimeRoot) {
  const applicationId = descriptor.manifest.id
  const indexed = readApplicationProposalListIndex({ runtimeRoot, applicationId, allowMissing: true })
  if (indexed && !hasApplicationProposalCreateIntentEntries(runtimeRoot, applicationId)) return
  return withApplicationRepositoryRecoveryClaim({
    runtimeRoot,
    repositoryRoot: descriptor.repositoryRoot,
    action: async () => {
      bootstrapProposalListIndex(runtimeRoot, applicationId)
      for (const handle of listApplicationProposalCreateIntents(runtimeRoot, applicationId)) {
...
export async function createApplicationProposal({
  ...
  return runGovernedCreateTransaction({
    ...
    releaseCreationIntent: (handle) => mutateCreateJournal(async () => {
      const indexed = indexDurableCreateArtifact(
        runtimeRoot,
        descriptor.manifest.id,
        handle.value.proposalId,
        handle.value.intentToken,
      )
      if (indexed) await transactionOperations.checkpoint?.("listing_index_published", { proposalId })
      releaseApplicationProposalCreateIntent(handle)
    }),
...
export function listApplicationProposalPage({ applicationId, runtimeRoot, requestedBy }) {
  const id = required(applicationId, "applicationId")
  if (!isApplicationId(id) || typeof runtimeRoot !== "string" || !path.isAbsolute(runtimeRoot)) {
    throw new Error("APPLICATION_PROPOSAL_ID_INVALID")
  }
  const index = readApplicationProposalListIndex({ runtimeRoot, applicationId: id })
  const proposals = index.proposalIds
    .map((proposalId) => getApplicationProposal({ applicationId: id, runtimeRoot, proposalId, requestedBy }))
    .sort((left, right) => right.createdAt.localeCompare(left.createdAt))
  return Object.freeze({ proposals: Object.freeze(proposals), truncated: index.truncated })
}

export function listApplicationProposals(options) {
  return listApplicationProposalPage(options).proposals
}
...
export async function rejectApplicationProposal({ application, runtimeRoot: configuredRuntime, proposalId, requestedBy, reason, transactionOperations = {} }) {
  ...
  return runGovernedRejectLifecycle({
    ...
    // Terminal replay never consults new caller text: the durable reason is
    // immutable and cleanup must remain resumable even if a retry body differs
    // or no longer satisfies current admission validation.
    claimReject: async (stored) => ({
      ...stored,
      rejectionReason: rejectedReason ??= rejectedBecause(reason),
    }),
