...
export function createApplicationExecutionRouting({
  ...
  const list = ({ externalEnabled = externalEnabledDefault(), externalCapability = true } = {}) => {
    const available = enabled(externalEnabled)
    return ROUTES.map((route) => ({ ...route, available: !route.external || (available && externalCapability === true) }))
  }
