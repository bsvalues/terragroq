...
function parseLock(bytes, expectedRepositoryDigest) {
  ...
  const keys = ["schemaVersion", "token", "processId", "processIdentity", "startedAt", "repositoryDigest", "proposalId"]
  if (!value || typeof value !== "object" || Array.isArray(value)
    ...
    || typeof value.processIdentity !== "string" || !/^(?:win|linux|posix):[A-Za-z0-9 .:+-]{1,120}$/.test(value.processIdentity)
...
function parseReaper(bytes, expectedRepositoryDigest, expectedName) {
  let value
  try { value = JSON.parse(bytes) } catch { throw new Error("APPLICATION_PROPOSAL_LOCK_UNCERTAIN") }
  const keys = ["schemaVersion", "token", "processId", "processIdentity", "startedAt", "repositoryDigest", "ticket"]
  if (!value || typeof value !== "object" || Array.isArray(value)
    || Object.keys(value).sort().join(",") !== keys.sort().join(",") || value.schemaVersion !== 1
...
function processStartIdentity(processId) {
  ...
  if (process.platform === "win32") {
    ...
    const output = execFileSync("ps", ["-o", "lstart=", "-p", String(processId)], {
      encoding: "utf8", timeout: 5_000, maxBuffer: 4_096,
    }).trim().replace(/\s+/g, " ")
    if (!output || output.length > 100) throw new Error()
    identity = `posix:${output}`
  }
...
export async function withApplicationRepositoryRecoveryClaim({ runtimeRoot, repositoryRoot, action, waitMs = REAPER_WAIT_MS }) {
  ...
  let value = { schemaVersion: 1, token, processId: process.pid, processIdentity: currentProcessIdentity(), startedAt: new Date().toISOString(), repositoryDigest, ticket: 0 }
  ...
  try {
    return await withApplicationRepositoryRecoveryClaim({ runtimeRoot, repositoryRoot, action: async () => {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        const value = { schemaVersion: 1, token: crypto.randomUUID(), processId: process.pid, processIdentity: currentProcessIdentity(), startedAt: new Date().toISOString(), repositoryDigest, proposalId }
