...
function exactPath(value) {
  return typeof value === "string" && value.length > 0 && value.length <= 240
    ...
    && value.split("/").every((segment) => SAFE_SEGMENT.test(segment) && segment !== "." && segment !== "..")
...
export function createProposalEngine(input) {
  ...
  if (!Array.isArray(input.allowedPaths) || input.allowedPaths.length < 1
    || input.allowedPaths.some((item) => !exactPath(item))
    || new Set(input.allowedPaths).size !== input.allowedPaths.length) throw new Error("APPLICATION_PROPOSAL_DESCRIPTOR_INVALID")
  if (!Array.isArray(input.validationPaths) || input.validationPaths.some((item) => !exactPath(item))
    || new Set(input.validationPaths).size !== input.validationPaths.length
    || input.allowedPaths.some((item) => !input.validationPaths.includes(item))) throw new Error("APPLICATION_PROPOSAL_DESCRIPTOR_INVALID")
  ...
  const allowedPaths = Object.freeze([...input.allowedPaths])
  const validationPaths = Object.freeze([...input.validationPaths])
