...
export async function resolveAuthorityRegistryUrl(databaseUrl, options = {}) {
  ...
  // as "the sentinel survived". What comes back out of the probe is the parser's own rendering,
  // whatever that is, which is what keeps this a test of ACCEPTANCE rather than of shape: measured
  // on this Node build, `postgresql:` is not a "special" scheme, so its host is parsed as an OPAQUE
  // host — no lower-casing and no IDNA — and a hand-rolled `toLowerCase()` comparison here would
  // have refused a registry that writes `ATLAS` even though the parser took it happily.
