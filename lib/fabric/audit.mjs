 ...
 *
 * PRESENCE IS NOT WRITABILITY, and the first version of this function conflated them. It checked that
 * the root existed and called that a guarantee, so a ledger that was present but unappendable -- a
 * read-only mount, a permissions change, an `audit.log` that is a directory -- passed the preflight,
 * the mutation ran, and only the append afterwards raised `AUDIT_UNAVAILABLE`. That is precisely the
 * loud-unrecorded-mutation this function exists to prevent, reintroduced one layer down.
 *
 * So the check is an actual zero-byte append. It creates the log if it is absent, which is the same
 * thing the first real write would do, and it fails for every reason a real write would fail. It adds
 * no line, so proving the ledger works cannot itself pollute it.
 *
 * A ZERO-BYTE APPEND IS NOT QUITE A WRITE, and the shape check above is there because of the one case
 * where that gap matters. On Node 24 an `appendFile(dir, "")` SUCCEEDS -- the open is permitted and
 * writing nothing is a no-op -- while `appendFile(dir, "x")` throws `EISDIR`. So an `audit.log` that
 * is a directory, the first example the review named, passed a preflight built to catch it and the
 * mutation ran; only the real append afterwards raised. Verified by execution rather than reasoned
 * about, and the same fixture the ledger tests already use is what surfaced it. Statting the path
 * first closes it: what is there must be a regular file, or there is no ledger to write to.
 */
export async function requireLedger(fabricRoot) {
  ...
  const log = path.join(fabricRoot, "audit.log")
  const existing = await fs.stat(log).catch(() => null)
  if (existing && !existing.isFile()) {
    throw new Error(`${AUDIT_UNAVAILABLE}: ledger at ${fabricRoot} is not a file`)
  }
  try {
    await fs.appendFile(log, "", "utf8")
  } catch {
    throw new Error(`${AUDIT_UNAVAILABLE}: ledger at ${fabricRoot} cannot be written`)
  }
}
