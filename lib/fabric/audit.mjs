...
async function hasLedger(fabricRoot) {
  if (!ledgerPresence.has(fabricRoot)) {
    ledgerPresence.set(fabricRoot, fs.access(fabricRoot).then(() => true).catch(() => false))
  }
  return ledgerPresence.get(fabricRoot)
}

export async function auditFabricAction(fabricRoot, node, action, rc, detail) {
  if (!(await hasLedger(fabricRoot))) return
  const line = [
