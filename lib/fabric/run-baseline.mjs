 ...
 *
 * `start` runs a workload, `push` writes a file there, and `stop` force-stops the workload and
 * deletes what it wrote. `reach` and `containers` only look, and `pull` reads the file back. These
 * three are the ones that must not proceed without a durable record.
 */
export const MUTATING_BASELINE_STEPS = new Set(["start", "push", "stop"])
...
export async function runNodeBaseline(name, node, options = {}) {
  const {
    ...
    registry,
    /**
     * Whether a mutating step may run without a writable ledger. Default NO.
     *
     * Defaulted on rather than off, and opting out has to be written down: a caller that forgets this
     * flag gets the safe behaviour, and the unsafe behaviour is only reachable by asking for it. The
     * previous shape -- brokered but not required -- was the failure this option exists to make
  ...
  // AND the mutating steps require their evidence up front. Routing through the broker was only half
  // the fix: `requireAudit` defaults to false, so a start / transfer / force-stop cycle would still
  // have run against a fabric root with no ledger and left nothing behind -- the exact outcome the
  // commit that introduced this claimed to have closed. `requireLedger` now proves the ledger can
  // actually be appended to, BEFORE the node is touched, so the guarantee is real in both halves.
  const sh = async (command) => {
    const { stdout } = await brokeredExec(name, command, {
      ...
      requireAudit: requireAudit && MUTATING_BASELINE_STEPS.has(currentStep),
    })
    return stdout
  }
