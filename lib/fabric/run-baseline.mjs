...
function sshArgs(node, command, fabricRoot) {
  return [
...
export async function runNodeBaseline(name, node, options = {}) {
  const { fabricRoot = defaultFabricRoot(), exec = defaultExec, stepTimeoutMs = STEP_TIMEOUT_MS } = options
  const results = []
  ...
  await step("containers", () => sh(windows ? "(docker ps -q | Measure-Object).Count" : "docker ps -q | wc -l"))
  await step("start", () => sh(
    windows
