 ...
 *
 * It deliberately does not reimplement the transport. `run-baseline.mjs` already knows how to survive
...
/** Append to the ledger the Python broker already writes, in its format: ts, node, action, rc, detail. */
export async function auditBrokerAction(fabricRoot, node, action, rc, detail) {
  const line = [
  ...
  try {
    ...
    // A ledger that cannot be written must not silently become a ledger that is not written: the
    // caller is told, and decides. Callers that treat auditing as optional are the reason the
    // application's actions were missing from it in the first place.
...
export async function brokeredExec(name, command, options = {}) {
  ...
  try {
    if (local) {
      ...
      await auditBrokerAction(fabricRoot, name, action, 0, command)
