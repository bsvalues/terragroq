...
function defaultRunner(command, args, options = {}) {
  return new Promise((resolve, reject) => execFile(command, args, { ...options, windowsHide: true, encoding: "utf8" }, (error, stdout, stderr) => {
...
export async function validateHelloApplicationInContainer({ repositoryRoot, runtimeRoot, workspacePath, commandRunner = defaultRunner }) {
  ...
  try {
    ...
    if (image?.code !== 0 || String(image?.stdout).trim() !== reviewed.build.imageId) throw new Error("image")
    const result = await commandRunner("docker", [
      ...
      reviewed.build.image, "--test", "examples/hello-application/test/hello.test.mjs",
    ], { env })
    if (result?.code !== 0) throw new Error("run")
