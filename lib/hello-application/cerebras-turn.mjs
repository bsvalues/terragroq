...
export function resolveCerebrasCredentialBridge({
  applicationRoot = process.env.WILLIAMOS_PROJECT_ROOT,
} = {}) {
  try {
    if (typeof applicationRoot !== "string" || !path.isAbsolute(applicationRoot) || applicationRoot.includes("\0")) {
      throw new Error()
    }
    ...
    if (!wrapperInfo.isFile() || wrapperInfo.isSymbolicLink() || wrapperInfo.nlink !== 1n
      || !samePath(resolvedWrapper, wrapper) || !resolvedWrapper.startsWith(`${root}${path.sep}`)) {
      throw new Error()
    }
