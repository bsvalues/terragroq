...
function boundedValidationFiles(root) {
  ...
  for (const item of VALIDATION_PATHS) {
    ...
    if (!Number.isSafeInteger(stat.size) || stat.size < 0 || stat.size >= MAX_SOURCE_FILE_BYTES) {
      throw new Error("HELLO_PROPOSAL_SOURCE_SIZE_REFUSED")
    }
    total += stat.size
    if (!Number.isSafeInteger(total) || total > MAX_SOURCE_TOTAL_BYTES) throw new Error("HELLO_PROPOSAL_SOURCE_SIZE_REFUSED")
    files.set(item, { target, mode: stat.mode & 0o777 })
  }
  return files
}
function snapshot(root, paths = VALIDATION_PATHS) {
  ...
  return new Map(paths.map((item) => {
    ...
    return [item, { bytes: fs.readFileSync(file.target), mode: file.mode }]
  }))
}
 ...
 */
export async function runGovernedResidentChange({ client, threadId, requestText, readChangedPaths, timeoutMs = TURN_TIMEOUT_MS, maximumAttempts = 3, verifyAttempt, now = Date.now }) {
  ...
  for (let attempt = 1; attempt <= limit; attempt++) {
    ...
    if (remaining <= 0) throw new Error("HELLO_PROPOSAL_RESIDENT_TIMEOUT")
    ...
    let turn
    let failure
    try { turn = await client.runTurn({ threadId, prompt: correction + governedPrompt(requested), timeoutMs: remaining }) }
...
export async function createHelloApplicationProposal({ repositoryRoot: configured, runtimeRoot, requestedBy, requestText, onProgress, residentTurn = defaultResident, validateWorkspace = validateHelloApplicationInContainer }) {
  ...
  let receiptPublicationStarted = false
  const files = pathsFor(runtime, id)
  try {
    ...
    receiptPublicationStarted = true
    ...
    if (!persisted) {
      if (receiptPublicationStarted) { try { fs.rmSync(files.receipt, { force: true }) } catch { /* Preserve the original failure. */ } }
      if (patchWritten) { try { fs.rmSync(files.patch, { force: true }) } catch { /* Preserve the original failure. */ } }
