...
function occursOnce(source, token) {
  const first = source.indexOf(token)
  return first >= 0 && source.indexOf(token, first + token.length) < 0
}

function assertGovernedMarkerSemantics(repositoryRoot) {
  const read = (relativePath) => fs.readFileSync(path.join(repositoryRoot, ...relativePath.split("/")), "utf8").replaceAll("\r\n", "\n")
  const html = read("examples/hello-application/src/index.html")
  const styles = read("examples/hello-application/src/styles.css")
  const app = read("examples/hello-application/src/app.js")
  const htmlContract = [
  ...
  const appLookup = '  const governanceMarker = root.getElementById("governance-marker")'
  const appCount = '    const pulseNumber = String(snapshot.count).padStart(3, "0")'
  const appCountOutput = "    countOutput.textContent = pulseNumber"
  const appUpdate = "    if (governanceMarker) governanceMarker.textContent = `Governed by HERMES · pulse ${pulseNumber}`"
  if (!html.includes(htmlContract)
    || !occursOnce(html, 'id="governance-marker"')
    || !occursOnce(styles, styleContract)
    || !occursOnce(app, appLookup)
    || !occursOnce(app, appCount)
    || !occursOnce(app, appCountOutput)
    || !occursOnce(app, appUpdate)) {
    throw new Error("HELLO_PROPOSAL_GOVERNED_MARKER_INVALID")
  }
}

async function validateHelloApplication(repositoryRoot) {
  assertGovernedMarkerSemantics(repositoryRoot)
