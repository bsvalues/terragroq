...
function hashFile(root, item) { const stat = fs.lstatSync(path.join(root, ...item.split("/"))); if (!stat.isFile() || stat.isSymbolicLink()) throw new Error("HELLO_PROPOSAL_WORKSPACE_FILE_INVALID"); return crypto.createHash("sha256").update(fs.readFileSync(path.join(root, ...item.split("/")))).digest("hex") }
...
async function applyInner({ repositoryRoot, runtimeRoot, proposalId, requestedBy, validateWorkspace }) {
  const repository = fs.realpathSync(path.resolve(repositoryRoot)); const runtime = path.resolve(runtimeRoot); const { value, files } = receipt(runtime, required(proposalId, "proposalId")); if (value.requestedBy !== required(requestedBy, "requestedBy") || value.status !== "READY_FOR_REVIEW") throw new Error("HELLO_PROPOSAL_NOT_APPLICABLE")
  assertHelloApplicationChangedPaths(value.changedPaths, []); const head = (await git(repository, ["rev-parse", "HEAD"])).stdout.trim(); if (head !== value.baseSha) throw new Error("HELLO_PROPOSAL_STALE_BASE")
  if ((await git(repository, ["status", "--porcelain=v1", "-z", "--", ...VALIDATION_PATHS])).stdout) throw new Error("HELLO_PROPOSAL_TARGET_DIRTY")
  const bytes = fs.readFileSync(files.patch); if (crypto.createHash("sha256").update(bytes).digest("hex") !== value.patchSha256) throw new Error("HELLO_PROPOSAL_PATCH_MISMATCH"); const paths = await patchPaths(repository, files.patch); if (JSON.stringify(paths) !== JSON.stringify(value.changedPaths)) throw new Error("HELLO_PROPOSAL_PATCH_SCOPE_MISMATCH")
  await git(repository, ["apply", "--check", "--whitespace=error-all", files.patch]); await git(repository, ["apply", "--whitespace=error-all", files.patch]); const validationWorkspace = path.join(runtime, "worktrees", `apply-${crypto.randomUUID()}`)
  try {
    fs.mkdirSync(path.dirname(validationWorkspace), { recursive: true }); await git(repository, ["worktree", "add", "--detach", validationWorkspace, value.baseSha]); direct(runtime, validationWorkspace); await git(validationWorkspace, ["apply", "--whitespace=error-all", files.patch]); regular(validationWorkspace, VALIDATION_PATHS)
    const validation = await validateWorkspace({ repositoryRoot: repository, runtimeRoot: runtime, workspacePath: validationWorkspace }); for (const item of paths) if (hashFile(repository, item) !== hashFile(validationWorkspace, item)) throw new Error("HELLO_PROPOSAL_VALIDATION_HASH_MISMATCH")
    const index = path.join(runtime, "worktrees", `apply-index-${crypto.randomUUID()}`); const env = { ...process.env, GIT_INDEX_FILE: index }
    try { await git(repository, ["read-tree", value.baseSha], { env }); await git(repository, ["add", "--", ...paths], { env }); const tree = (await git(repository, ["write-tree"], { env })).stdout.trim(); const commit = (await git(repository, ["-c", "user.name=WilliamOS HERMES Apply", "-c", "user.email=hermes@williamos.local", "commit-tree", tree, "-p", value.baseSha, "-m", `apply(hello): governed proposal ${proposalId}`], { env })).stdout.trim(); await git(repository, ["update-ref", "HEAD", commit, value.baseSha]); for (const item of paths) { const blob = (await git(repository, ["hash-object", item])).stdout.trim(); await git(repository, ["update-index", "--add", "--cacheinfo", `100644,${blob},${item}`]) }; const updated = { ...value, status: "APPLIED", appliedAt: new Date().toISOString(), appliedCommit: commit, validation }; writeJson(files.receipt, updated); return review(updated, files) } finally { try { fs.rmSync(index, { force: true }) } catch {} }
  } catch (error) { await git(repository, ["apply", "--reverse", files.patch], { allowFailure: true }); throw error } finally { await removeWorktree(repository, runtime, validationWorkspace) }
}
