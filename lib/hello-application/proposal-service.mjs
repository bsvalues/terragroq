...
export async function createHelloApplicationProposal({ repositoryRoot: configured, runtimeRoot, requestedBy, requestText,
  ...
  return runGovernedCreateTransaction({
    ...
    prepareCandidateIndex: (target, lockTarget) => {
      ...
      const prefix = `${path.resolve(workspace)}.candidate-index-`
      const parent = path.dirname(resolved)
      const expectedLock = `${resolved}.lock`
      const parentStat = fs.lstatSync(parent)
      if (!resolved.startsWith(prefix) || !ID.test(resolved.slice(prefix.length))
        || path.resolve(lockTarget) !== expectedLock
        || parent !== path.resolve(runtime, "worktrees") || fs.existsSync(resolved) || fs.existsSync(expectedLock)
        || parentStat.isSymbolicLink() || fs.realpathSync(parent) !== parent) {
        throw new Error("HELLO_PROPOSAL_WORKTREE_INVALID")
      }
    },
    assertCandidateIndex: (target, lockTarget) => {
      ...
      if (!resolved.startsWith(prefix) || !ID.test(resolved.slice(prefix.length))
        || path.resolve(lockTarget) !== expectedLock || fs.existsSync(expectedLock)
        || path.dirname(resolved) !== path.resolve(runtime, "worktrees") || !stat.isFile()
        || stat.isSymbolicLink() || stat.nlink !== 1n || fs.realpathSync(resolved) !== resolved) {
        throw new Error("HELLO_PROPOSAL_WORKTREE_INVALID")
      }
    },
    cleanupCandidateIndex: (target, lockTarget) => {
      ...
      const expectedLock = `${resolved}.lock`
      if (!resolved.startsWith(prefix) || !ID.test(resolved.slice(prefix.length))
        || path.resolve(lockTarget) !== expectedLock || path.dirname(resolved) !== path.resolve(runtime, "worktrees")) {
        throw new Error("HELLO_PROPOSAL_WORKTREE_CLEANUP_FAILED")
      }
      if (fs.existsSync(expectedLock)) {
        const lockStat = fs.lstatSync(expectedLock, { bigint: true })
        if (!lockStat.isFile() || lockStat.isSymbolicLink() || lockStat.nlink !== 1n
          || fs.realpathSync(expectedLock) !== expectedLock) {
          throw new Error("HELLO_PROPOSAL_WORKTREE_CLEANUP_FAILED")
        }
        fs.unlinkSync(expectedLock)
        if (fs.existsSync(expectedLock)) throw new Error("HELLO_PROPOSAL_WORKTREE_CLEANUP_FAILED")
      }
      if (fs.existsSync(resolved)) {
        const indexStat = fs.lstatSync(resolved)
        if (!indexStat.isFile() || indexStat.isSymbolicLink()) throw new Error("HELLO_PROPOSAL_WORKTREE_CLEANUP_FAILED")
        fs.unlinkSync(resolved)
        if (fs.existsSync(resolved)) throw new Error("HELLO_PROPOSAL_WORKTREE_CLEANUP_FAILED")
      }
    },
