...
async function assertGovernedMarkerSemantics(repositoryRoot) {
  const baselineByPath = {}
  for (const relativePath of HELLO_APPLICATION_ALLOWED_PATHS) {
    baselineByPath[relativePath] = (await git(repositoryRoot, ["show", `HEAD:${relativePath}`], {
...
async function validateHelloApplication(repositoryRoot) {
  await assertGovernedMarkerSemantics(repositoryRoot)
