...
function pathsFor(runtime, id) {
  ...
  return { root, receipt: path.join(root, `${id}.json`), patch: path.join(root, `${id}.patch`), quarantine: path.join(root, `${id}.quarantine`) }
}
...
function snapshot(root, paths = VALIDATION_PATHS) {
  return new Map(paths.map((item) => {
    ...
    return [item, { bytes: fs.readFileSync(target), mode: fs.statSync(target).mode & 0o777 }]
  }))
}
...
/**
 * @param {{ client: { runTurn: (input: { threadId: string, prompt: string, timeoutMs: number }) => Promise<any> }, threadId: string, requestText: string, readChangedPaths: () => Promise<string[]>, timeoutMs?: number, maximumAttempts?: number, verifyAttempt?: (outcome: any) => any }} options
 * @returns {Promise<{ turn: any, changedPaths: string[], attempts: number }>}
 */
export async function runGovernedResidentChange({ client, threadId, requestText, readChangedPaths, timeoutMs = TURN_TIMEOUT_MS, maximumAttempts = 3, verifyAttempt }) {
  ...
  for (let attempt = 1; attempt <= limit; attempt++) {
    ...
    let turn
    let failure
    try { turn = await client.runTurn({ threadId, prompt: correction + governedPrompt(requested), timeoutMs }) }
    catch (error) { if (!recoverable(error)) throw error; failure = error }
    ...
    if (!failure) return { turn, changedPaths: paths, attempts: attempt }
...
function receipt(runtime, id) {
  ...
  const target = fs.existsSync(files.quarantine) ? files.quarantine : files.receipt
  let bytes
  try { bytes = fs.readFileSync(target) }
  catch (error) { throw new Error(error.code === "ENOENT" ? "HELLO_PROPOSAL_NOT_FOUND" : "HELLO_PROPOSAL_RECEIPT_INVALID") }
...
export async function createHelloApplicationProposal({ repositoryRoot: configured, runtimeRoot, requestedBy, requestText, onProgress, residentTurn = defaultResident, validateWorkspace = validateHelloApplicationInContainer }) {
  ...
  try {
    ...
    fs.writeFileSync(files.patch, bytes, { flag: "wx" })
    await patchPaths(repository, files.patch)
    ...
    const result = review(value, files)
    ...
    if (!persisted) await git(repository, ["branch", "-D", branch], { allowFailure: true })
  }
}
...
async function applyInner({ repositoryRoot: configured, runtimeRoot, proposalId, requestedBy, validateWorkspace, transactionOperations = {} }) {
  ...
  try {
    ...
    // All cleanup and checks precede the atomic receipt rename. Return uses verified memory only.
    await removeWorktree(repository, runtime, workspace)
    ...
    try {
      ...
      if (written.size) {
        ...
        for (const item of synced) {
          const current = (await indexEntries(repository)).find((entry) => entryPath(entry) === item)
          const owned = candidateEntries.get(item)
          const initial = originalIndex.find((entry) => entryPath(entry) === item)
          if (current !== initial) {
            if (current !== `${owned.mode} ${owned.blob} 0\t${item}`) throw new Error("unowned index state")
...
export function listHelloApplicationProposals({ runtimeRoot, requestedBy }) {
  ...
  const ids = [...new Set(fs.readdirSync(root).filter((name) => /\.(?:json|quarantine)$/.test(name)).map((name) => name.replace(/\.(?:json|quarantine)$/, "")))]
