...
function validReceipt(value, id) {
  if (!value || ![1, 2].includes(value.schemaVersion) || value.proposalId !== id || !ID.test(id)
    || !["READY_FOR_REVIEW", "APPLY_IN_PROGRESS", "APPLIED", "QUARANTINED_ROLLBACK_FAILED"].includes(value.status)
  ...
  if (value.status === "APPLY_IN_PROGRESS") {
    if (value.schemaVersion !== 2 || !timestamp(value.applyStartedAt) || value.applyStartedAt < value.createdAt) throw new Error()
...
function receipt(runtime, id) {
  ...
  if (inflight) {
    ...
    const { applyStartedAt, ...projection } = inflight.value
    projection.status = "READY_FOR_REVIEW"
...
async function applyInner({ repositoryRoot: configured, runtimeRoot, proposalId, requestedBy, validateWorkspace, transactionOperations = {} }) {
  ...
  let proposed
  let updated
  const inflight = value.schemaVersion === 2
    ? { ...value, status: "APPLY_IN_PROGRESS", applyStartedAt: new Date().toISOString() }
    : { ...value, status: "QUARANTINED_ROLLBACK_FAILED", quarantinedAt: new Date().toISOString() }
  validReceipt(inflight, proposalId)
  let inflightWriteStarted = false
  ...
  try {
    ...
    try {
      ...
      if (inflightWriteStarted && fs.existsSync(files.inflight)) {
        let observed
        try { observed = validReceipt(JSON.parse(fs.readFileSync(files.inflight)), proposalId) }
        catch { throw new Error("journal cleanup uncertain") }
        if (!equal(observed, inflight) || !removeVerified(files.inflight)) throw new Error("journal cleanup uncertain")
      }
...
export async function applyHelloApplicationProposal(options) {
  const current = receipt(options.runtimeRoot, required(options.proposalId, "proposalId")).value
  if (current.requestedBy !== required(options.requestedBy, "requestedBy")) throw new Error("HELLO_PROPOSAL_OWNER_MISMATCH")
  if (current.status !== "READY_FOR_REVIEW") throw new Error("HELLO_PROPOSAL_NOT_APPLICABLE")
