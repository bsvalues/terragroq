...
function releaseClaim(claim) {
  ...
  if (!removeVerified(claim.files.inflight)) throw new Error("claim cleanup uncertain")
  let current
  try { current = receipt(claim.runtime, claim.proposalId) }
  catch { throw new Error("claim cleanup uncertain") }
  if (!current.bytes.equals(Buffer.from(claim.originalReceiptBase64, "base64"))
    || !equal(current.value, claim.value) || current.value.status !== "READY_FOR_REVIEW") {
    throw new Error("claim cleanup uncertain")
  }
}
function bestEffortReleaseAppliedClaim(claim) {
  try {
    if (fs.readFileSync(claim.files.inflight, "utf8") === claim.markerText) removeVerified(claim.files.inflight)
