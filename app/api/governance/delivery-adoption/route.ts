...
export async function POST(request: Request): Promise<Response> {
  ...
  const preview = body.mode === "PREVIEW" && Object.keys(body).every((key) => ["mode", "worldId"].includes(key))
    && typeof body.worldId === "string" && body.worldId.trim().length > 0
  const mutation = (body.mode === "AUTHORIZE" || body.mode === "ISSUE")
    && Object.keys(body).every((key) => ["mode", "worldId", "confirmedPreviewDigest", "idempotencyKey"].includes(key))
    && typeof body.worldId === "string" && body.worldId.trim().length > 0
    && (body.mode === "ISSUE" || (typeof body.confirmedPreviewDigest === "string" && DIGEST.test(body.confirmedPreviewDigest)))
    && (body.mode !== "ISSUE" || body.confirmedPreviewDigest === undefined)
  ...
  try {
    if (body.mode === "PREVIEW") return reply(await previewPersistedArtifactAdoption(session.user.id, String(body.worldId).trim()))
    const args = [session.user.id, String(body.worldId).trim(), String(body.idempotencyKey), String(body.confirmedPreviewDigest)] as const
    if (body.mode === "AUTHORIZE") return reply(await authorizePersistedArtifactAdoption(...args), 201)
