...
if (process.env.WILLIAMOS_OPERATOR_RESTORE === "1") {
  const restored = await call(`${BASE}/api/resource/restore`, { identity: "PACS" }, sessionCookie)
  console.log(`POST /api/resource/restore       -> HTTP ${restored.status}`)
  console.log((await restored.text()).slice(0, 500))
}
