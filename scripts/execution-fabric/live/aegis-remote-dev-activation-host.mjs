...
const PRECLAIM_PREDECESSOR_COMMIT = "070b93f1fe2fbf1e9f2072c7c150e329618878c9"
...
export function inspectPreparedSnapshotRecovery(v){try{if(!exactKeys(v,["preparedCommit","currentCommit","entries","claimExists","phaseExists","sessionExists"])||!Array.isArray(v.entries)||v.claimExists||v.phaseExists||v.sessionExists||!/^([a-f0-9]{40})$/.test(v.preparedCommit)||!/^([a-f0-9]{40})$/.test(v.currentCommit))throw new Error();const entries=[...v.entries].sort(),base=["control","mints","prepared.json"].sort(),predecessor=[...base,"no-sudo.json"].sort(),allowed=new Set([...base,"no-sudo.json","network-intent.json","network-applied.json","network-settled.json","activation-failure.json"]);if(v.preparedCommit===v.currentCommit&&base.every(x=>entries.includes(x))&&entries.every(x=>allowed.has(x)))return"MATCH";if(v.preparedCommit===PRECLAIM_PREDECESSOR_COMMIT&&JSON.stringify(entries)===JSON.stringify(predecessor))return"ARCHIVE_EXACT_PRECLAIM";throw new Error()}catch{return"DRIFT"}}
