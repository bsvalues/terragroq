...
export const ACTIVATION_ID = "remote-dev-offload-v1-issue-734-single-use-003"
export const AUTHORITY_REFERENCE = "issue-734-terrafusion-remote-dev-single-use-003"
...
const PRE_RECEIPT_EXPECTED_KEYS=["authorityId","authoritySha256","authorityIssuedAt","authorityExpiresAt","machineIdSha256","bootId","controlCommit","activationHostSha256","compactionIntentSha256","compactionSha256","minimumAvailableBytes"]
export function diagnosePreReceiptCompactionReceipt(v,e){
  const keys=["schemaVersion","status","runId",...PRE_RECEIPT_EXPECTED_KEYS,"availableBytes","completedAt","executionAuthorized","activationAuthorized"]
  if(!exactKeys(e,PRE_RECEIPT_EXPECTED_KEYS))return{status:"DRIFT",reasonCode:"EXPECTED_KEYS_DIFFER"}
  if(!exactKeys(v,keys))return{status:"DRIFT",reasonCode:"RECEIPT_KEYS_DIFFER"}
  if(v.schemaVersion!==1||v.status!=="TRANSIENT_ACTIVATION_COMPACTION_VERIFIED"||v.runId!==RUN_ID)return{status:"DRIFT",reasonCode:"RECEIPT_IDENTITY_DIFFERS"}
  if(Object.entries(e).some(([key,value])=>v[key]!==value))return{status:"DRIFT",reasonCode:"RECEIPT_BINDING_DIFFERS"}
  if(![v.authoritySha256,v.machineIdSha256,v.activationHostSha256,v.compactionIntentSha256,v.compactionSha256].every(value=>typeof value==="string"&&SHA.test(value))||!GUID.test(v.authorityId)||!GUID.test(v.bootId)||!/^[a-f0-9]{40}$/.test(v.controlCommit))return{status:"DRIFT",reasonCode:"RECEIPT_FORMAT_DIFFERS"}
  const completed=Date.parse(v.completedAt),issued=Date.parse(e.authorityIssuedAt),expires=Date.parse(e.authorityExpiresAt)
  if(![completed,issued,expires].every(Number.isFinite)||completed<issued||completed>=expires)return{status:"DRIFT",reasonCode:"COMPLETION_TIME_OUTSIDE_AUTHORITY"}
  if(!Number.isSafeInteger(v.availableBytes)||v.minimumAvailableBytes!==PRE_RECEIPT_MINIMUM_AVAILABLE_BYTES||v.availableBytes<v.minimumAvailableBytes)return{status:"DRIFT",reasonCode:"POST_CAPACITY_INSUFFICIENT"}
  if(v.executionAuthorized!==false||v.activationAuthorized!==false)return{status:"DRIFT",reasonCode:"AUTHORIZATION_FLAGS_DIFFER"}
  return{status:"MATCH",reasonCode:"PRE_RECEIPT_COMPACTION_RECEIPT_MATCHED"}
}
export function inspectPreReceiptCompactionReceipt(v,e){return diagnosePreReceiptCompactionReceipt(v,e).status}
export function buildPreReceiptCompactionReceipt(expected,availableBytes,completedAt){return{schemaVersion:1,status:"TRANSIENT_ACTIVATION_COMPACTION_VERIFIED",runId:RUN_ID,...expected,availableBytes,completedAt,executionAuthorized:false,activationAuthorized:false}}
export function buildPreReceiptCompactionExpectation(authority,proof){return{...authority,compactionIntentSha256:proof.intentSha256,compactionSha256:proof.compactionSha256,minimumAvailableBytes:PRE_RECEIPT_MINIMUM_AVAILABLE_BYTES}}
...
export function diagnoseNetworkSliceStartResult(value){if(!exactKeys(value,["status","signal","errorCode","stdout","stderr"]))return"NETWORK_SLICE_RESULT_SCHEMA_INVALID";if(value.errorCode!==null)return"NETWORK_SLICE_PROCESS_FAILED";if(value.signal!==null)return"NETWORK_SLICE_SIGNALLED";if(value.status!==0)return"NETWORK_SLICE_EXIT_STATUS_FAILED";if(value.stdout!=="")return"NETWORK_SLICE_STDOUT_UNEXPECTED";if(value.stderr!=="")return"NETWORK_SLICE_STDERR_UNEXPECTED";return"NETWORK_SLICE_START_VERIFIED"}
export function diagnoseNetworkActivePostcondition(value){if(!exactKeys(value,["nftExact","egressActiveEnabled","brokerInactiveDisabled","gitSocketInactiveDisabled","gitServiceInactive","listenerAbsent","workerWorkspaceAbsent"])||Object.values(value).some(entry=>typeof entry!=="boolean"))return"NETWORK_POSTCONDITION_SCHEMA_INVALID";if(!value.nftExact)return"NETWORK_NFT_POSTCONDITION_FAILED";if(!value.egressActiveEnabled)return"NETWORK_EGRESS_POSTCONDITION_FAILED";if(value.brokerInactiveDisabled)return"NETWORK_BROKER_START_FAILED";if(value.gitSocketInactiveDisabled)return"NETWORK_GIT_SOCKET_START_FAILED";if(!value.gitServiceInactive)return"NETWORK_GIT_SERVICE_UNEXPECTED";if(value.listenerAbsent)return"NETWORK_LISTENER_START_FAILED";if(!value.workerWorkspaceAbsent)return"NETWORK_WORKER_OR_WORKSPACE_UNEXPECTED";return"NETWORK_ACTIVE_POSTCONDITION_VERIFIED"}
...
function ensureNetworkSlice(){
  const result=startNetworkSliceResult(),diagnosis=diagnoseNetworkSliceStartResult(result);if(diagnosis!=="NETWORK_SLICE_START_VERIFIED")throw new Error(diagnosis)
  let s;try{s=fs.statSync(`/sys/fs/cgroup${NETWORK_SLICE}`,{bigint:true})}catch{throw new Error("NETWORK_SLICE_CGROUP_STAT_FAILED")}if(!s.isDirectory())throw new Error("NETWORK_SLICE_CGROUP_INVALID");return{device:s.dev.toString(),inode:s.ino.toString(),ctimeNs:s.ctimeNs.toString()}
}
...
function ensureNetworkBoundaryReceipt(){
  ...
  try{
    if(state==="ACTIVATE_EXACT_INERT"){activationAttempted=true;try{run("/usr/bin/systemctl",["enable","--now","williamos-aegis-remote-dev-broker.service","williamos-aegis-remote-dev-git-broker.socket"])}catch{throw new Error("NETWORK_UNIT_ENABLE_FAILED")}try{observed=networkObservation()}catch{throw new Error("NETWORK_ACTIVE_OBSERVATION_FAILED")}const diagnosis=diagnoseNetworkActivePostcondition(observed);if(diagnosis!=="NETWORK_ACTIVE_POSTCONDITION_VERIFIED")throw new Error(diagnosis)}
...
function compactMain(authorityPath) {
  ...
  const receiptExpected=proof=>buildPreReceiptCompactionExpectation({authorityId:p.authorityId,authoritySha256,authorityIssuedAt:p.issuedAt,authorityExpiresAt:p.expiresAt,machineIdSha256:p.machineIdSha256,bootId:p.bootId,controlCommit:p.controlCommit,activationHostSha256:authorityExpected.activationHostSha256},proof)
  ...
  const proof=proveTransientCompactionComplete(),availableBytes=availableRunBytes(),completedAt=trustedNow(),expected=receiptExpected(proof),receipt=buildPreReceiptCompactionReceipt(expected,availableBytes,completedAt),diagnostic=diagnosePreReceiptCompactionReceipt(receipt,expected)
  if(diagnostic.status!=="MATCH")throw new Error(diagnostic.reasonCode)
