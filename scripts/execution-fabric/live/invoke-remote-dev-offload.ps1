...
'@
    ...
    try { $sessionHandle = [Text.Encoding]::UTF8.GetString($ticketBytes) | ConvertFrom-Json -Depth 5 } catch { Write-ResultAndExit 'INVALID_INPUT' 'ACTIVATION_SESSION_INVALID' 'activation session handle is malformed' 64 }
...
function InvokeActivation([hashtable]$request,[string]$code,[switch]$NoFail){$b64=[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes(($request|ConvertTo-Json -Compress -Depth 10)));$ap=[Diagnostics.ProcessStartInfo]::new();$ap.FileName=$ssh;$ap.UseShellExecute=$false;$ap.CreateNoWindow=$true;$ap.RedirectStandardOutput=$true;$ap.RedirectStandardError=$true;$ap.Arguments='-o BatchMode=yes -o ConnectTimeout=10 -o ConnectionAttempts=1 -o StrictHostKeyChecking=yes -o UserKnownHostsFile="'+$knownHosts+'" aegis activation '+$b64;$p=[Diagnostics.Process]::new();$p.StartInfo=$ap;if(-not$p.Start()){if($NoFail){return $null}else{Fail $code 'activation request did not start'}};$out=$p.StandardOutput.ReadToEndAsync();$err=$p.StandardError.ReadToEndAsync();if(-not$p.WaitForExit(15000)){try{$p.Kill()}catch{};if($NoFail){return $null}else{Fail $code 'activation request timed out'}};$text=$out.GetAwaiter().GetResult();if($p.ExitCode-ne0){if($NoFail){return $null}else{Fail $code 'activation request failed'}};try{$value=$text|ConvertFrom-Json -Depth 30}catch{if($NoFail){return $null}else{Fail $code 'activation response is malformed'}};return @{Text=$text;Value=$value}}
...
$script:activationStarted=$true;$started=InvokeActivation ([ordered]@{action='start'}) 'ACTIVATION_START_FAILED';if($started.Value.payload.runId-ne$packet.runId-or$started.Value.signature-notmatch'^[A-Za-z0-9+/]+={0,2}$'){Fail 'ACTIVATION_START_FAILED' 'activation session binding differs'}
...
$mintPsi=[Diagnostics.ProcessStartInfo]::new();$mintPsi.FileName=$ssh;$mintPsi.UseShellExecute=$false;$mintPsi.CreateNoWindow=$true;$mintPsi.RedirectStandardOutput=$true;$mintPsi.RedirectStandardError=$true;$mintPsi.Arguments='-o BatchMode=yes -o ConnectTimeout=10 -o ConnectionAttempts=1 -o StrictHostKeyChecking=yes -o UserKnownHostsFile="'+$knownHosts+'" aegis activation '+$mintB64
...
'@
    ...
    $relayBootstrap = @'
$ErrorActionPreference='Stop';$expected='__RELAY_SHA__';$expectedEnvelope='__ENVELOPE_SHA__';$envelopePath='__ENVELOPE_PATH__';$relayPath='__RELAY_PATH__';try{$item=Get-Item -LiteralPath $envelopePath -Force;if(($item.Attributes-band[IO.FileAttributes]::ReparsePoint)-ne0-or$item.PSIsContainer-or$item.Length-lt2-or$item.Length-gt2097152){exit 64};$envelopeBytes=[IO.File]::ReadAllBytes($envelopePath);$hash=[Security.Cryptography.SHA256]::Create();try{$envelopeActual=([BitConverter]::ToString($hash.ComputeHash($envelopeBytes))).Replace('-','').ToLowerInvariant()}finally{$hash.Dispose()};if($envelopeActual-ne$expectedEnvelope){exit 64};$envelope=[Text.Encoding]::UTF8.GetString($envelopeBytes)|ConvertFrom-Json;if((@($envelope.PSObject.Properties.Name|Sort-Object)-join',')-ne'relayGzip,relayInput,relaySha256'-or$envelope.relaySha256-ne$expected){exit 64};$raw=[Convert]::FromBase64String($envelope.relayGzip);$source=[IO.MemoryStream]::new([byte[]]$raw);$gzip=[IO.Compression.GZipStream]::new($source,[IO.Compression.CompressionMode]::Decompress);$target=[IO.MemoryStream]::new();try{$gzip.CopyTo($target)}finally{$gzip.Dispose();$source.Dispose()};$bytes=$target.ToArray();$target.Dispose();$hash=[Security.Cryptography.SHA256]::Create();try{$actual=([BitConverter]::ToString($hash.ComputeHash($bytes))).Replace('-','').ToLowerInvariant()}finally{$hash.Dispose()};if($actual-ne$expected){exit 64};[IO.File]::WriteAllBytes($relayPath,$bytes);$psi=[Diagnostics.ProcessStartInfo]::new();$psi.FileName='powershell.exe';$psi.UseShellExecute=$false;$psi.CreateNoWindow=$true;$psi.RedirectStandardInput=$true;$psi.RedirectStandardOutput=$true;$psi.RedirectStandardError=$true;$psi.Arguments='-NoProfile -NonInteractive -File "'+$relayPath+'"';$p=[Diagnostics.Process]::new();$p.StartInfo=$psi;if(-not$p.Start()){exit 2};$out=$p.StandardOutput.ReadToEndAsync();$err=$p.StandardError.ReadToEndAsync();$p.StandardInput.Write([Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($envelope.relayInput)));$p.StandardInput.Close();if(-not$p.WaitForExit(5400000)){try{$p.Kill()}catch{};exit 2};[Console]::Out.Write($out.GetAwaiter().GetResult());[Console]::Error.Write($err.GetAwaiter().GetResult());exit $p.ExitCode}finally{Remove-Item -LiteralPath $relayPath -Force -ErrorAction SilentlyContinue;Remove-Item -LiteralPath $envelopePath -Force -ErrorAction SilentlyContinue}
'@
    ...
    $cleanupScript = '$ErrorActionPreference=''Stop'';$relay='''+$remoteRelayPath.Replace('/','\')+''';$envelope='''+$remoteTransportPath.Replace('/','\')+''';$all=@(Get-CimInstance Win32_Process);$ids=[Collections.Generic.HashSet[int]]::new();foreach($target in @($all|Where-Object{$_.ProcessId-ne$PID-and$_.CommandLine-and$_.CommandLine.Contains($relay)})){[void]$ids.Add([int]$target.ProcessId)};do{$added=$false;foreach($child in $all){if($ids.Contains([int]$child.ParentProcessId)-and$ids.Add([int]$child.ProcessId)){$added=$true}}}while($added);foreach($id in @($ids)|Sort-Object -Descending){Stop-Process -Id $id -Force -ErrorAction SilentlyContinue};Start-Sleep -Milliseconds 250;Remove-Item -LiteralPath $relay,$envelope -Force -ErrorAction SilentlyContinue;if(Test-Path -LiteralPath $relay){exit 64};if(Test-Path -LiteralPath $envelope){exit 64};if(@(Get-CimInstance Win32_Process|Where-Object{$_.ProcessId-ne$PID-and$_.CommandLine-and$_.CommandLine.Contains($relay)}).Count-ne0){exit 64}'
    ...
    if ($remote.ExitCode -ne 0) {
        ...
        if ($remote.ExitCode -eq 2 -and $failureLines.Count -eq 1) { try { $cleanupFailure = $failureLines[0] | ConvertFrom-Json -Depth 40 } catch { $cleanupFailure = $null } }
