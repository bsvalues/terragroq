...
$trustedWorkerSha256 = '155f32a21f0be1ffa15c31e763f20d7c656e7e1027c3e0eeec6cc626f190ad04'
...
if($process.ExitCode-ne0){
  ...
  if($validRecoverable){
    ...
    if($attempt -ge [int]$packet.resourceLimits.maxAttempts){$state.terminalStatus='BLOCKED';$state.terminalReason='CLEANUP_RECOVERY_EXHAUSTED';SaveState;[Console]::Out.WriteLine((@{status='BLOCKED';reasonCode='CLEANUP_RECOVERY_EXHAUSTED';detail='cleanup recovery attempt budget exhausted';runId=$packet.runId;operation='CLEAN_EXACT_WORKSPACE';attempt=$attempt;previousEvidenceSha256=$expectedPrevious;headSha=$recoverable.headSha;failureResultSha256=$failureDigest;causeCode=$recoverable.causeCode}|ConvertTo-Json -Compress));exit 2}
