...
if($process.ExitCode-ne0){
  ...
  if($validRecoverable){$failureDigest=CanonicalDigest $workerLines[0];$failureRecord=[ordered]@{attempt=$attempt;branch=$packet.branch;causeCode=$recoverable.causeCode;headSha=$recoverable.headSha;operation='CLEAN_EXACT_WORKSPACE';originalAbsent=$true;packetSha256=$packetDigest;policySha256=$policyDigest;previousEvidenceSha256=$expectedPrevious;quarantinePath=$expectedQuarantine;reasonCode='CLEANUP_QUARANTINED_RECOVERABLE';resultSha256=$failureDigest;runId=$packet.runId};$state.lastAttempt=$attempt;$state.inFlightOperation=$null;$state.terminalStatus='RECOVERABLE_CLEAN_BLOCKED';$state.terminalReason='CLEANUP_QUARANTINED_RECOVERABLE';$state.recoverableHeadSha=$recoverable.headSha;$state.cleanupFailures=@($state.cleanupFailures)+@($failureRecord);SaveState;[Console]::Out.WriteLine((@{status='BLOCKED';reasonCode='CLEANUP_QUARANTINED_RECOVERABLE';detail='quarantined cleanup requires same-run retry';runId=$packet.runId;operation='CLEAN_EXACT_WORKSPACE';attempt=$attempt;previousEvidenceSha256=$expectedPrevious;headSha=$recoverable.headSha;failureResultSha256=$failureDigest;causeCode=$recoverable.causeCode}|ConvertTo-Json -Compress));exit 2}
  $state.terminalStatus='BLOCKED';$state.terminalReason='AEGIS_WORKER_FAILED';SaveState;if($stdout.Trim()){[Console]::Out.WriteLine($stdout.Trim());exit 2}else{Fail 'AEGIS_WORKER_FAILED' ('AEGIS worker exit '+$process.ExitCode)}
}
