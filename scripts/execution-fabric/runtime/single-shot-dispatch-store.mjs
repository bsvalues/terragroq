...
function readState(filePath, storeId, clock) {
  if (!fs.existsSync(filePath)) return initialState(storeId, clock)
  try { return validateState(JSON.parse(fs.readFileSync(filePath, "utf8")), storeId) } catch (error) {
...
export function createSingleShotDispatchStore(filePath, {
  ...
  const mutate = (operation) => withLock(absolute, () => {
    const state = readState(absolute, storeId, clock)
    ...
    atomicWrite(absolute, validateState(state, storeId))
    return result
  }, clock, staleLockMs)
  ...
  return Object.freeze({
    read: () => structuredClone(readState(absolute, storeId, clock)),
