...
| `run-hermes-doctrine.ps1` | SYSTEM-task entry point (vendored per #1141 review): observe -> normalize -> evaluate -> write result; records a truthful `DOCTRINE_NOT_DECLARED` UNKNOWN when no doctrine.json exists yet. Never mutates declared doctrine. |
...
## The asymmetry rule (adversarial-review finding, 2026-09-02; hardened #1141)
...
a Codex desktop lane's dev server and was correctly held as drift until the process went away
and it self-cleared. No baseline was re-minted. Comparator fixes (e.g. the `nvcontainer` loopback UDP
exemption, 2026-09-02) land here as behavior changes with contract tests, never by editing
`doctrine.json` to match noise.

Corollary from the #1141 review (P1): a process NAME is never an ingress excuse. Agent/browser
owners are ephemeral only for their genuine background shapes — UDP on ephemeral/mDNS ports, or
TCP bound strictly to loopback. An agent-owned TCP socket on `0.0.0.0` or a LAN address is
externally reachable ingress and always fails the gate, whatever binary owns it.
