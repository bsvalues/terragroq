...
try {
  Assert-LegacyAdapterQuarantined -RegistryPath $registry
}
catch {
  $wall = if ($_.Exception.Message -match "[A-Z][A-Z0-9_]+_WALL|QUARANTINED_TERMINAL") { $Matches[0] } else { "LEGACY_ADAPTER_QUARANTINE_INTEGRITY_WALL" }
